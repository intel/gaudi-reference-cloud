...
kubectl -n ${namespace} exec ${dbpostgrespod} -- bash -c "PGPASSWORD=${post_pass} psql --set=sslmode=${dbsslmode} -U ${dbuser} -d ${dbname} -h ${dbhost} -p ${dbport} -c 'SELECT * FROM defaultconfig'"

echo ""

read -p "Type the key to update ie. cp_cloudaccountid: " inputkey
read -p "Type the value to update ie. 098349980597: " inputvalue 

inputkey=$(echo "'${inputkey}'")
inputvalue=$(echo "'${inputvalue}'")
runcmd=$(echo "UPDATE defaultconfig set value=${inputvalue} WHERE name=${inputkey}")
echo $runcmd
