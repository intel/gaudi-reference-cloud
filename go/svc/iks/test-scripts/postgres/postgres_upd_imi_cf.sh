...
kubectl -n ${namespace} exec ${dbpostgrespod} -- bash -c "PGPASSWORD=${post_pass} psql --set=sslmode=${dbsslmode} -U ${dbuser} -d ${dbname} -h ${dbhost} -p ${dbport} -c 'SELECT osimageinstance_name, instancetypecategory, instancetypefamiliy FROM osimageinstance'"

echo ""

read -p "Type the imi to update ie. iks-u22-cd-cp-1-27-4-23-09-19: " inputkey
read -p "Type the field to update (category/family): " inputfield
read -p "Type the field value .i.e. VirtualMachine: " inputvalue

if [ "${inputfield}" == "family" ]; then
    inputfield="familiy"
fi

inputkey=$(echo "'${inputkey}'")
inputvalue=$(echo "'${inputvalue}'")
runcmd=$(echo "UPDATE osimageinstance set instancetype${inputfield}=${inputvalue} WHERE osimageinstance_name=${inputkey}")
echo $runcmd
