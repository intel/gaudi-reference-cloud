...
var (
	standardDriverConn *grpc.ClientConn
	Test               TestService
)
...
func (ts *TestService) Init(ctx context.Context, cfg *Config,
	...
	addr, err := resolver.Resolve(ctx, "billing-standard")
	if err != nil {
		return err
	}
	if standardDriverConn, err = grpc.Dial(addr, grpc.WithTransportCredentials(insecure.NewCredentials())); err != nil {
		return err
	}
	return nil
}
