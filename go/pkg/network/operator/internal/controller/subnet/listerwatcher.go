...
type ListerWatcher struct {
	grpcClient pb.SubnetPrivateServiceClient
...
func NewListerWatcher(grpcClient pb.SubnetPrivateServiceClient, timeout time.Duration) *ListerWatcher {
...
func (lw *ListerWatcher) List(options metav1.ListOptions) (runtime.Object, error) {
	...
	ctx, log, span := obs.LogAndSpanFromContext(ctx).WithName("SubnetListerWatcher.List").Start()
	defer span.End()
	log.V(9).Info("BEGIN", "options", options)
	defer log.V(9).Info("END")
	var subnets []subnetv1alpha1.Subnet
	...
	stream, err := lw.grpcClient.SearchStreamPrivate(ctx, &pb.SubnetSearchStreamPrivateRequest{})
	if err != nil {
		return nil, err
	}
	if stream == nil {
...
func (lw *ListerWatcher) Watch(options metav1.ListOptions) (watch.Interface, error) {
	ctx := context.Background()
	ctx, log, span := obs.LogAndSpanFromContext(ctx).WithName("SubnetListerWatcher.Watch").Start()
	defer span.End()
	log.V(9).Info("BEGIN", "options", options)
	defer log.V(9).Info("END")
	...
	idleTimer := idletimer.New(logAndCancel)
	idleTimer.Reset(lw.timeout)
	stream, err := lw.grpcClient.Watch(ctx, &pb.SubnetWatchRequest{
