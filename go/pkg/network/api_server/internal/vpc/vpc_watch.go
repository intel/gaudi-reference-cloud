...
func (s *VPCService) SearchStreamPrivate(req *pb.VPCSearchStreamPrivateRequest, svc pb.VPCPrivateService_SearchStreamPrivateServer) error {
	ctx := svc.Context()
	log := log.FromContext(ctx).WithName("VPCService.SearchStreamPrivate")
	err := func() error {
		...
		selectSql := fmt.Sprintf("select %s from vpc", s.sqlTransformer.ColumnsForFromRow())
...
func (s *VPCService) Watch(req *pb.VPCWatchRequest, svc pb.VPCPrivateService_WatchServer) error {
	ctx := svc.Context()
	log := log.FromContext(ctx).WithName("VPCService.Watch")
	err := func() error {
		log.Info("Request", logkeys.Request, req)
		...
		// Resource version was converted by rowToVPCPrivate from an integer in the database to a string.
		...
		for {
			...
			// This prevents the response stream from being detected as idle by the VPC Replicator.
...
func (s *VPCService) getMaximumResourceVersion(ctx context.Context) (int64, error) {
	// Get max resourceVersion over entire table.
	var row *sql.Row
	if row = s.db.QueryRowContext(ctx, "select coalesce(max(resource_version), 0) from vpc"); row.Err() != nil {
		return 0, row.Err()
	}
...
func (s *VPCService) watchSendRows(ctx context.Context, svc pb.VPCPrivateService_WatchServer, rows *sql.Rows) error {
	log := log.FromContext(ctx).WithName("VPCService.watchSendRows")
	recordCount := int64(0)
	for rows.Next() {
		vpc, err := s.sqlTransformer.FromRowWatchResponse(ctx, rows)
		if err != nil {
			return err
		}
		resp := pb.VPCWatchResponse{
			Type:   pb.WatchDeltaType_Updated,
			Object: vpc,
		}
		if vpc.Metadata.DeletedTimestamp != nil {
...
func (s *VPCService) watchSendBookmark(ctx context.Context, svc pb.VPCPrivateService_WatchServer, resourceVersion int64) error {
	resp := pb.VPCWatchResponse{
