...
// This returns all non-deleted subnets as messages with WatchDeltaType=Updated,
...
func (s *SubnetService) SearchStreamPrivate(req *pb.SubnetSearchStreamPrivateRequest, svc pb.SubnetPrivateService_SearchStreamPrivateServer) error {
	ctx := svc.Context()
	log := log.FromContext(ctx).WithName("SubnetService.SearchStreamPrivate")
	err := func() error {
		...
		selectSql := fmt.Sprintf("select %s from subnet", transformer.ColumnsForFromRow())
...
// Return a stream of changes to subnets using messages with WatchDeltaType=Updated or Deleted.
...
func (s *SubnetService) Watch(req *pb.SubnetWatchRequest, svc pb.SubnetPrivateService_WatchServer) error {
	ctx := svc.Context()
	log := log.FromContext(ctx).WithName("SubnetService.Watch")
	err := func() error {
		log.Info("Request", logkeys.Request, req)
		...
		// Resource version was converted by rowToSubnetPrivate from an integer in the database to a string.
		...
		for {
			...
			// This prevents the response stream from being detected as idle by the Subnet Replicator.
...
func (s *SubnetService) getMaximumResourceVersion(ctx context.Context) (int64, error) {
	// Get max resourceVersion over entire table.
	var row *sql.Row
	if row = s.db.QueryRowContext(ctx, "select coalesce(max(resource_version), 0) from subnet"); row.Err() != nil {
		return 0, row.Err()
	}
...
func (s *SubnetService) watchSendRows(ctx context.Context, svc pb.SubnetPrivateService_WatchServer, rows *sql.Rows) error {
	log := log.FromContext(ctx).WithName("SubnetService.watchSendRows")
	recordCount := int64(0)
	for rows.Next() {
		subnet, err := s.sqlTransformer.FromRowWatchResponse(ctx, rows)
		if err != nil {
			return err
		}
		resp := pb.SubnetWatchResponse{
			Type:   pb.WatchDeltaType_Updated,
			Object: subnet,
		}
		if subnet.Metadata.DeletedTimestamp != nil {
...
func (s *SubnetService) watchSendBookmark(ctx context.Context, svc pb.SubnetPrivateService_WatchServer, resourceVersion int64) error {
	resp := pb.SubnetWatchResponse{
		Type: pb.WatchDeltaType_Bookmark,
		Object: &pb.SubnetPrivateWatchResponse{
