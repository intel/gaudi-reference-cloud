...
// List load balancers as a stream.
...
func (lb *Service) SearchStreamPrivate(req *pb.LoadBalancerSearchStreamPrivateRequest, svc pb.LoadBalancerPrivateService_SearchStreamPrivateServer) error {
	ctx := svc.Context()
	log := log.FromContext(ctx).WithName("LoadBalancerService.SearchStreamPrivate")
	err := func() error {
		log.Info("Request", "req", req)

		maxResourceVersionAtStart, err := lb.getMaximumResourceVersion(ctx)
		if err != nil {
			return err
		}
		log.Info("Starting", logkeys.MaxResourceVersionAtStart, maxResourceVersionAtStart)

		selectSql := fmt.Sprintf("select %s from loadbalancer", lb.sqlTransformer.ColumnsForFromRow())
		// Do not send deleted records.
		query := selectSql + " where resource_version <= $1 and deleted_timestamp = $2"
		rows, err := lb.db.QueryContext(ctx, query, maxResourceVersionAtStart, common.TimestampInfinityStr)
		if err != nil {
			return err
		}
		defer rows.Close()
		if err := lb.watchSendRows(ctx, svc, rows); err != nil {
			return err
		}
		...
		if err := lb.watchSendBookmark(ctx, svc, maxResourceVersionAtStart); err != nil {
			return err
		}
		return nil
	}()
...
// Used by Load Balancer Replicator.
// This is modeled after https://kubernetes.io/docs/reference/using-api/api-concepts/#efficient-detection-of-changes.
...
func (lb *Service) Watch(req *pb.LoadBalancerWatchRequest, svc pb.LoadBalancerPrivateService_WatchServer) error {
	ctx := svc.Context()
	log := log.FromContext(ctx).WithName("LoadBalancerService.Watch")
	err := func() error {
		log.Info("Request", "req", req)
		...
		stmt, err := lb.db.PrepareContext(ctx, selectSql+" where resource_version > $1 and resource_version <= $2")
		if err != nil {
			return err
		}
		defer stmt.Close()
		...
		for {
			maxResourceVersion, err := lb.getMaximumResourceVersion(ctx)
			if err != nil {
				return err
			}
			...
			// This prevents the response stream from being detected as idle by the Load Balancer Replicator.
			if err := lb.watchSendBookmark(ctx, svc, maxResourceVersion); err != nil {
				return err
			}
...
func (lb *Service) getMaximumResourceVersion(ctx context.Context) (int64, error) {
	// Get max resourceVersion over entire table.
	var row *sql.Row
	if row = lb.db.QueryRowContext(ctx, "select coalesce(max(resource_version), 0) from loadbalancer"); row.Err() != nil {
		return 0, row.Err()
	}
...
func (lb *Service) watchSendRows(ctx context.Context, svc pb.LoadBalancerPrivateService_WatchServer, rows *sql.Rows) error {
	log := log.FromContext(ctx).WithName("LoadBalancerService.watchSendRows")
	recordCount := int64(0)
	for rows.Next() {
		...
		resp := pb.LoadBalancerWatchResponse{
			Type:   pb.WatchDeltaType_Updated,
			Object: loadbalancer,
		}
		if loadbalancer.Metadata.DeletedTimestamp != nil {
...
func (lb *Service) watchSendBookmark(ctx context.Context, svc pb.LoadBalancerPrivateService_WatchServer, resourceVersion int64) error {
	resp := pb.LoadBalancerWatchResponse{
