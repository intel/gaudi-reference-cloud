...
import (
	...
	"github.com/intel-innersource/frameworks.cloud.devcloud.services.idc/go/pkg/productcatalog_operator/controllers"
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "1b6b7695.intel.com",
	...
	if err = (&controllers.VendorReconciler{
	...
	var billingSyncClient pb.BillingProductCatalogSyncServiceClient
	if err != nil {
		setupLog.Error(err, "unable to resolve billingsync address", "controller", "Product")
		billingSyncClient = nil
	} else {
		conn, err := grpcutil.NewClient(ctx, addr)
		if err != nil {
			setupLog.Error(err, "unable to obtain connection for billingsync", "controller", "Product")
			billingSyncClient = nil
		} else {
			billingSyncClient = pb.NewBillingProductCatalogSyncServiceClient(conn)
		}
	}
	...
	if err = (&controllers.ProductReconciler{
		Client:            mgr.GetClient(),
		Scheme:            mgr.GetScheme(),
		BillingSyncClient: billingSyncClient,
	}).SetupWithManager(mgr); err != nil {
