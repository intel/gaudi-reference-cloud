...
func calculateUsages(t *testing.T, ctx context.Context, cloudAcctId string, meteringQuantity float32) {
	...
	// todo: these can be global variables.
...
func TestStandardCloudCreditReport(t *testing.T) {
	...
	cloudCreditUsageReport := NewCloudCreditUsageReport(standardDriverTesting.GetDriverDb(), pb.NewCloudAccountServiceClient(cloudAccountConn),
		pb.NewUsageServiceClient(usageConn))
	cloudCreditUsageReport.ReportCloudCreditUsages(ctx)
...
func TestIntelCloudCreditReport(t *testing.T) {
	...
	calculateUsages(t, ctx, cloudAcct.Id, 120)
	cloudCreditUsageReport := NewCloudCreditUsageReport(intelDriverTesting.GetDriverDb(), pb.NewCloudAccountServiceClient(cloudAccountConn),
		pb.NewUsageServiceClient(usageConn))
	cloudCreditUsageReport.ReportCloudCreditUsages(ctx)
