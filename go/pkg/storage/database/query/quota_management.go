...
const (
	ScopeAccountId   ScopeType = "ACCOUNT_ID"
	ScopeAccountType ScopeType = "ACCOUNT_TYPE"
)
...
func DeleteServiceResourceQuota(ctx context.Context, tx *sql.Tx, serviceId, resourceName string) error {
	...
	rows, err := tx.ExecContext(ctx, deleteServiceResourceQuota, serviceId, resourceName)
	if err != nil {
		return err
	}
