...
import (
	...
	meteringMonitor "github.com/intel-innersource/frameworks.cloud.devcloud.services.idc/go/pkg/storage/bucket_metering_monitor/metering_monitor"
...
func main() {
	...
	err := func() error {
		...
		cfg := &privatecloudv1alpha1.BucketMeteringMonitorConfig{}
		...
		if err != nil {
			log.Info("unable to init storage controller client")
			log.Error(err, "strCntrClient")
		}
		log.Info("Storage controller server address", logkeys.ServerAddr, serverAddr)
		ctx := context.Background()
		...
		_, err = meteringMonitor.NewMeteringMonitor(ctx, k8sManager, meteringClient, cfg, &strCntrClient)
		if err != nil {
