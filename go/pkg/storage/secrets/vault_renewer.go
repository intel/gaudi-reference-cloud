...
func (v *Vault) renewVaultToken(ctx context.Context) bool {
	...
	go func(chan bool) {
		...
		if err != nil {
			log.Error(err, "failed to renew the token", logkeys.RoleId, v.roleID)
		} else {
			renewalStatus <- status
			log.Info("renewal status", logkeys.Status, status)
			wg.Done()
		}
	}(renewalStatus)
	defer func() {
...
func (v *Vault) tokenRenewer(ctx context.Context) (error, bool) {
	...
	log.Info("token properties", logkeys.TokenTTL, tokenTTL,
		logkeys.RenewalTime, v.renewalTime,
		logkeys.TimeSinceRenewalTime, time.Since(v.renewalTime))
	...
	for {
		...
		if status {
			log.Info("renewed vault token", logkeys.RoleId, v.roleID)
		} else {
			log.Info("failed to renew vault token", logkeys.RoleId, v.roleID)
		}
		if err == nil {
			return nil, status
		}
...
func (v *Vault) manageTokenLifecycle(ctx context.Context) (error, bool) {
	...
	for {
		...
		case err := <-authTokenWatcher.DoneCh():
			...
			if err != nil {
				log.Info("failed to renew token. re-attempting login.", logkeys.RoleId, v.roleID)
			}
			log.Info("token can no longer be renewed. re-attempting login.", logkeys.RoleId, v.roleID)
			return nil, false
		...
		case renewal := <-authTokenWatcher.RenewCh():
			...
			if !renewal.RenewedAt.IsZero() {
				log.Info("renewed token", logkeys.RoleId, v.roleID, logkeys.RenewalTime, renewal.RenewedAt)
