...
func removeFromSlice(ctx context.Context, s []*PodInfo, k string) []*PodInfo {
	log := log.FromContext(ctx).WithName("removeFromSlice")

	for i := range s {
		k2, err := GetPodKey(s[i].Pod)
		if err != nil {
			log.Error(err, "Cannot get pod key", logkeys.Pod, s[i].Pod)
			continue
		}
		if k == k2 {
			// delete the element
...
func (n *NodeInfo) RemovePod(ctx context.Context, pod *v1.Pod) error {
	log := log.FromContext(ctx).WithName("NodeInfo.RemovePod")
	k, err := GetPodKey(pod)
	if err != nil {
		return err
	}
	if podWithAffinity(pod) {
		n.PodsWithAffinity = removeFromSlice(ctx, n.PodsWithAffinity, k)
	}
	if podWithRequiredAntiAffinity(pod) {
		n.PodsWithRequiredAntiAffinity = removeFromSlice(ctx, n.PodsWithRequiredAntiAffinity, k)
	}

	for i := range n.Pods {
		k2, err := GetPodKey(n.Pods[i].Pod)
		if err != nil {
			log.Error(err, "Cannot get pod key", logkeys.Pod, n.Pods[i].Pod)
			continue
		}
		if k == k2 {
			// delete the element
...
const ResourceIdPodLabel string = "vm.kubevirt.io/name"

// GetPodKey returns the string key of a pod.
// This is used to match scheduled pods with running pods so that the reserved resources are not double counted.
func GetPodKey(pod *v1.Pod) (string, error) {
	// Pods created by Kubevirt for instances will have label vm.kubevirt.io/name set to the resourceId (UUID).
	// This will match with the assumed pod.Name.
	if pod.Labels != nil {
		resourceId, ok := pod.Labels[ResourceIdPodLabel]
		if ok {
			return resourceId, nil
		}
	}
	// Otherwise, return the pod.UID.
	uid := string(pod.UID)
	if len(uid) == 0 {
		return "", fmt.Errorf("cannot get cache key for pod with empty UID; pod.Name=%q", pod.Name)
	}
	return uid, nil
}
