...
func (pl *PodTopologySpread) calPreFilterState(ctx context.Context, pod *v1.Pod) (*preFilterState, error) {
	log := log.FromContext(ctx).WithName("PodTopologySpread.calPreFilterState")
...
func (pl *PodTopologySpread) Filter(ctx context.Context, cycleState *framework.CycleState, pod *v1.Pod, nodeInfo *framework.NodeInfo) *framework.Status {
	log := log.FromContext(ctx).WithName("PodTopologySpread.Filter")
	node := nodeInfo.Node()
	if node == nil {
	...
	for _, c := range s.Constraints {
		...
		if !ok {
			log.V(3).Info("Node doesn't have required label", logkeys.Node, node, logkeys.Label, tpKey)
		...
		if err != nil {
			log.Error(err, "Internal error occurred while retrieving value precalculated in PreFilter", logkeys.TopologyKey, tpKey, logkeys.KeyPath, s.TpKeyToCriticalPaths)
			continue
		}
		...
		if skew > int(c.MaxSkew) {
			log.V(3).Info("Node failed spreadConstraint: matchNum + selfMatchNum - minMatchNum > maxSkew", logkeys.Node, node, logkeys.TopologyKey, tpKey, "matchNum", matchNum, "selfMatchNum", selfMatchNum, "minMatchNum", minMatchNum, "maxSkew", c.MaxSkew)
