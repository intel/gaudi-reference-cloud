...
func TestNodeTree_UpdateNode(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			nt.updateNode(ctx, oldNode, test.nodeToUpdate)
			verifyNodeTree(t, nt, test.expectedTree)
		})
	}
}
			...
			for _, op := range test.operations {
				...
				case "add":
					if addIndex >= len(test.nodesToAdd) {
						...
						nt.addNode(ctx, test.nodesToAdd[addIndex])
						addIndex++
					}
				case "remove":
					if removeIndex >= len(test.nodesToRemove) {
						...
						nt.removeNode(ctx, test.nodesToRemove[removeIndex])
						removeIndex++
					}
				default:
