	...
	for i, tt := range tests {
		t.Run(fmt.Sprintf("case_%d", i), func(t *testing.T) {
			...
			for _, pod := range tt.pods {
				if err := cache.AssumePod(ctx, pod); err != nil {
			...
			for _, pod := range tt.pods {
				if err := cache.ForgetPod(ctx, pod); err != nil {
...
func assumeAndFinishBinding(cache *cacheImpl, pod *v1.Pod, assumedTime time.Time) error {
	...
	return cache.finishBinding(ctx, pod, assumedTime)
}
...
func TestExpirePod(t *testing.T) {
	...
	for i, tt := range tests {
		t.Run(fmt.Sprintf("case_%d", i), func(t *testing.T) {
			...
			for _, pod := range tt.pods {
				if err := cache.AssumePod(ctx, pod.pod); err != nil {
					t.Fatal(err)
				}
				if !pod.finishBind {
					continue
				}
				if err := cache.finishBinding(ctx, pod.pod, pod.assumedTime); err != nil {
					t.Fatal(err)
				}
			}
			// pods that got bound and have assumedTime + ttl < cleanupTime will get
			// expired and removed
			cache.cleanupAssumedPods(ctx, tt.cleanupTime)
			n := cache.nodes[nodeName]
			if err := deepEqualWithoutGeneration(n, tt.wNodeInfo); err != nil {
				t.Error(err)
			}
	...
	for i, tt := range tests {
		t.Run(fmt.Sprintf("case_%d", i), func(t *testing.T) {
			...
			for _, podToAdd := range tt.podsToAdd {
				if err := cache.AddPod(ctx, podToAdd); err != nil {
					t.Fatalf("AddPod failed: %v", err)
				}
			}
			cache.cleanupAssumedPods(ctx, now.Add(2*ttl))
	...
	for i, tt := range tests {
		t.Run(fmt.Sprintf("case_%d", i), func(t *testing.T) {
			...
			for _, podToUpdate := range tt.podsToUpdate {
				if err := cache.UpdatePod(ctx, podToUpdate[0], podToUpdate[1]); err != nil {
...
func TestAddPodAfterExpiration(t *testing.T) {
	...
	for i, tt := range tests {
		t.Run(fmt.Sprintf("case_%d", i), func(t *testing.T) {
			...
			if err := cache.AddPod(ctx, tt.pod); err != nil {
				t.Fatalf("AddPod failed: %v", err)
			}
			// check after expiration. confirmed pods shouldn't be expired.
			n := cache.nodes[nodeName]
	...
	for i, tt := range tests {
		t.Run(fmt.Sprintf("case_%d", i), func(t *testing.T) {
			...
			for j := range tt.podsToUpdate {
				...
				if err := cache.UpdatePod(ctx, tt.podsToUpdate[j-1], tt.podsToUpdate[j]); err != nil {
					t.Fatalf("UpdatePod failed: %v", err)
				}
				// check after expiration. confirmed pods shouldn't be expired.
				n := cache.nodes[nodeName]
...
func TestUpdatePodAndGet(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			handler: func(cache Cache, pod *v1.Pod) error {
				return cache.AssumePod(ctx, pod)
			},
			assumePod: true,
		},
		{
	...
	for i, tt := range tests {
		t.Run(fmt.Sprintf("case_%d", i), func(t *testing.T) {
			...
			if !tt.assumePod {
				if err := cache.UpdatePod(ctx, tt.pod, tt.podToUpdate); err != nil {
					t.Fatalf("UpdatePod failed: %v", err)
				}
			}
...
func TestEphemeralStorageResource(t *testing.T) {
	...
	for i, tt := range tests {
		t.Run(fmt.Sprintf("case_%d", i), func(t *testing.T) {
			...
			if err := cache.RemovePod(ctx, tt.pod); err != nil {
...
func TestRemovePod(t *testing.T) {
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if tt.assume {
				if err := cache.AddPod(ctx, pod); err != nil {
					t.Fatalf("AddPod failed: %v", err)
				}
			} else {
				if err := cache.AssumePod(ctx, pod); err != nil {
			...
			if err := cache.RemovePod(ctx, pod); err != nil {
				t.Fatalf("RemovePod failed: %v", err)
			}
...
func TestNodeOperators(t *testing.T) {
	...
	for i, test := range tests {
		t.Run(fmt.Sprintf("case_%d", i), func(t *testing.T) {
			...
			if err := cache.UpdateSnapshot(ctx, cachedNodes); err != nil {
				t.Error(err)
			}
			...
			if err := cache.RemoveNode(ctx, node); err != nil {
				t.Error(err)
			}
			...
			for _, p := range test.pods {
				if err := cache.RemovePod(ctx, p); err != nil {
					t.Error(err)
				}
				if _, err := cache.GetPod(p); err == nil {
...
func TestSchedulerCache_UpdateSnapshot(t *testing.T) {
	...
	addNode := func(i int) operation {
		return func(t *testing.T) {
			cache.AddNode(ctx, nodes[i])
		}
	}
	removeNode := func(i int) operation {
		return func(t *testing.T) {
			if err := cache.RemoveNode(ctx, nodes[i]); err != nil {
				t.Error(err)
			}
		}
	}
	updateNode := func(i int) operation {
		return func(t *testing.T) {
			cache.UpdateNode(ctx, nodes[i], updatedNodes[i])
		}
	}
	addPod := func(i int) operation {
		return func(t *testing.T) {
			if err := cache.AddPod(ctx, pods[i]); err != nil {
				t.Error(err)
			}
		}
	}
	...
	addPodWithAffinity := func(i int) operation {
		return func(t *testing.T) {
			if err := cache.AddPod(ctx, podsWithAffinity[i]); err != nil {
				t.Error(err)
			}
		}
	}
	removePod := func(i int) operation {
		return func(t *testing.T) {
			if err := cache.RemovePod(ctx, pods[i]); err != nil {
				t.Error(err)
			}
		}
	}
	removePodWithAffinity := func(i int) operation {
		return func(t *testing.T) {
			if err := cache.RemovePod(ctx, podsWithAffinity[i]); err != nil {
				t.Error(err)
			}
		}
	}
	updatePod := func(i int) operation {
		return func(t *testing.T) {
			if err := cache.UpdatePod(ctx, pods[i], updatedPods[i]); err != nil {
				t.Error(err)
			}
		}
	}
	updateSnapshot := func() operation {
		return func(t *testing.T) {
			cache.UpdateSnapshot(ctx, snapshot)
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if err := cache.UpdateSnapshot(ctx, snapshot); err != nil {
				t.Error(err)
			}
			if err := compareCacheWithNodeInfoSnapshot(t, cache, snapshot); err != nil {
				t.Error(err)
...
func TestSchedulerCache_updateNodeInfoSnapshotList(t *testing.T) {
	...
	updateSnapshot := func(t *testing.T) {
		cache.updateNodeInfoSnapshotList(ctx, snapshot, true)
		if err := compareCacheWithNodeInfoSnapshot(t, cache, snapshot); err != nil {
			t.Error(err)
		}
	}
...
func BenchmarkUpdate1kNodes30kPods(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		cachedNodes := NewEmptySnapshot()
		cache.UpdateSnapshot(ctx, cachedNodes)
	}
}
...
func benchmarkExpire(b *testing.B, podNum int) {
	...
	for n := 0; n < b.N; n++ {
		...
		b.StartTimer()
		cache.cleanupAssumedPods(ctx, now.Add(2*time.Second))
	}
}
