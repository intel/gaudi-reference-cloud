...
import (
	...
	"github.com/intel-innersource/frameworks.cloud.devcloud.services.idc/go/pkg/kubernetes_reconciler/controllers"
...
func main() {
	...
	flag.StringVar(&operatorConfig, "operator-config", "/etc/kubernetes-reconciler", "This is the path to the operator config.")
	opts := zap.Options{
		Development: true,
	}
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "d409cb51.intel.com",
	...
	if len(config.IKSAPIAddress) == 0 {
		setupLog.Error(err, "IKS API Address empty")
		os.Exit(1)
	}

	// Create IKS API client.
	IKSAPIClient, err := iks.NewClient(config.IKSAPIAddress)
	if err != nil {
		setupLog.Error(err, "create IKS API client")
		os.Exit(1)
	}
	...
	// This will run the reconciler to watch for cluster status.
