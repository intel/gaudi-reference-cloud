...
// BAZEL_EXTRA_OPTS="--test_output=streamed --test_arg=-test.v --test_arg=-ginkgo.vv --test_env=ZAP_LOG_LEVEL=-127 //go/pkg/fleet_admin_ui_server/api_server/test/..." make test-custom
package test
...
var (
	...
	fleetAdminUIServiceClient pb.FleetAdminUIServiceClient
)
...
var _ = BeforeSuite(func() {
	...
	fleetAdminUIServiceClient = pb.NewFleetAdminUIServiceClient(clientConn)

	By("Pinging service until it comes up")
	Eventually(func(g Gomega) {
		_, err := fleetAdminUIServiceClient.Ping(ctx, &emptypb.Empty{})
