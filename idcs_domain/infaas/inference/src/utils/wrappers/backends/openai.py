...
class OpenAiWrapper(AbstractWrapper):
    ...
    _PROB_ENDPOINT = "chat/completions"
    ...
    def __init__(
        ...
        """
        Initiates a OpenAI async client and exposes its streaming.
        ...
        super().__init__(config, base_url, model_id, headers, cookies, timeout)
    ...
    async def is_prompt_safe(
        ...
        safety_response = await self._client.chat.completions.create(
    ...
    async def generate_stream(
        ...
        # OpenAI numeric params must be greater than zero or None
        # gRPC by default would give 0 to optional not populated fields
        openAI_np = self._NumericParams(
            requestID=requestID,
            config=self._config,
        ...
        raise NotImplementedError(
            "openai wrapper doesn't support `generate_stream` at the moment"
        )

        # # OpenAI client has an internal validation of the generation params
        # # If a validation fails - validator will raise a 'ValidationError'
        # stream_iterator = self._client.completions.create(
        #     prompt=prompt,
        #     max_tokens=openAI_np.max_new_tokens,
        #     frequency_penalty=openAI_np.frequency_penalty,
        #     seed=openAI_np.seed,
        #     stop_sequences=stop_sequences,
        #     temperature=openAI_np.temperature,
        #     top_p=openAI_np.top_p,
        # )
        # # TODO: add stream_options
        #
