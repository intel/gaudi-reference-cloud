...
dry_run=${DRYRUN:-0}
DOCKER=${DOCKER:-0}
VENV_NAME=${VENV_NAME:-bench_tf_resnet50_infer}

if [ "$DOCKER" != "0" ]; then
    LOG_DIR=${WORKSPACE}/bench_tf_resnet50_infer_container_logs
    WORK_DIR=${WORKSPACE}/bench_tf_resnet50_infer_container_workdir
else
    LOG_DIR=${WORKSPACE}/bench_tf_resnet50_infer_logs
    WORK_DIR=${WORKSPACE}/bench_tf_resnet50_infer_workdir
fi
mkdir -p ${LOG_DIR}
mkdir -p ${WORK_DIR}
...
validate_ai_resnet50_tf_infer(){
    ...
    if [ $tiles == 1 ]; then
        ...
        reference_data=(19500 39000 0 79000 0 0 0 158000)
    elif [ $tiles == 2 ]; then
        #reference dat for Max 1550
        num_of_card=(1 2 3 4 5 6 7 8)
        reference_data=(45000 90000 0 180000 0 0 0 360000)  
    fi
    perf_val=(0 0 0 0 0 0 0 0)

    #running in benchmark folder
    test_id="tf_resnet50_inference"
    cp -r ${SCRIPT_DIR}/../../workloads/AI/ResNet50_v1.5/tensorflow/inference/* ${WORK_DIR}
    bench_script_dir=${WORK_DIR}
    date4log=""
    #$(date '+%Y%m%d_%H%M%S')
    if [ "$DOCKER" != "0" ]; then
        script2run=./tf_resnet50_inference_container_run.sh
        log_file_prefix=${LOG_DIR}/tf_resnet50_inference_container_run
	script2setup=./tf_resnet50_inference_container_setup.sh
	log_file_setup=${LOG_DIR}/tf_resnet50_inference_container_setup_${date4log}.log
    else
	script2run=./tf_resnet50_inference_run.sh
        log_file_prefix=${LOG_DIR}/tf_resnet50_inference_run
	script2setup=./tf_resnet50_inference_setup.sh
	log_file_setup=${LOG_DIR}/tf_resnet50_inference_setup_${date4log}.log
    
    fi
    cd $bench_script_dir

    #setup environments
    print_prefix "AI benchmark evaluation - Tensorflow Resnet50 v1.5 Inference on $device_name"
    ...
    print_prefix "AI benchmark evaluation - Tensorflow Resnet50 v1.5 Inference DATAVALIDATION on GPU $device_name"

    #single card perf as baseline for scalability
    val_base=${perf_val[0]}
    if [ -z $val_base ];then val_base=0; else  val_base=$(printf '%.1f' $val_base); fi
    for i in $( seq 0 7); do
        val=${perf_val[$i]}        
        if [ -z $val ];then val=0; else val=$(printf '%.1f' ${perf_val[$i]}); fi
        vali=$(bc -l <<< "$val != 0")
        if (( $vali )); then
