        ...
        while len(pods_done) < len(pods):
            ...
            for p in pods:
                ...
                if p_name not in pods_done and state == "Running":
                    cmd     = f"kubectl cp -n {namespace} {cwd} {p_name}:/workdir/ighs_hccl_demo"
                    output  = run_cmd(cmd).strip()
...
class BareMetalUtils(SystemUtils):
    ...
    def initialize_node_jobs(self, level,
        ...
        for i, node_group in enumerate(node_groups):
            if level == 1:
                ...
                copy_files(src="tmp/config.yaml", dst=f"{self.remote_path}/ighs_hccl_demo", hosts=update_val["target-nodes"])
    ...
    def update_yaml_job(self,
        ...
        if yaml_type == "job":
            ...
            template_data["system-info"]["hostfile"] = f"/tmp/ighs/ighs_hccl_demo/{os.path.basename(hostfile)}"
            template_data["system-info"]["ssh-path"] = f"/tmp/ighs/ighs_hccl_demo/{os.path.basename(ssh_path)}"
