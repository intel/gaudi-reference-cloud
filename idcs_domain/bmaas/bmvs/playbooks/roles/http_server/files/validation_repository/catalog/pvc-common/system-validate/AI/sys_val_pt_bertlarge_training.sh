...
dry_run=${DRYRUN:-0}
DOCKER=${DOCKER:-0}
VENV_NAME=${VENV_NAME:-bench_pt_bertlarge_training}
DATASET_DIR=${DATASET_DIR:-/data0/mlcommons_bert}
PROCESSED_DATASET_DIR=${PROCESSED_DATASET_DIR:-/data0/mlcommons_bert/processed_hdf5_pytorch}

# add dataset dir check as the training requires real data set to run
export DATASET_DIR
export PROCESSED_DATASET_DIR
if [ ! -d $DATASET_DIR ] || [ ! -d $PROCESSED_DATASET_DIR ]; then
    echo "DATASET_DIR=$DATASET_DIR or PROCESSED_DATASET_DIR=$PROCESSED_DATASET_DIR not exist, create the folder first"
    exit
fi

if [ "$DOCKER" != "0" ]; then
    LOG_DIR=${WORKSPACE}/bench_pt_bertlarge_training_container_logs
    WORK_DIR=${WORKSPACE}/bench_pt_bertlarge_training_container_workdir
else
    LOG_DIR=${WORKSPACE}/bench_pt_bertlarge_training_logs
    WORK_DIR=${WORKSPACE}/bench_pt_bertlarge_training_workdir
fi
mkdir -p ${LOG_DIR}
mkdir -p ${WORK_DIR}
...
validate_pt_bertlarge_training(){
    ...
    if [ $tiles == 1 ]; then
        ...
        reference_data=(50 88 0 165 0 0 0 280)
    elif [ $tiles == 2 ]; then
        #reference dat for Max 1550
        num_of_card=(1 2 3 4 5 6 7 8)
        reference_data=(110 200 0 400 0 0 0 800)
    fi
    perf_val=(0 0 0 0 0 0 0 0)

    #running in benchmark folder
    test_id="pt_bertlarge_training"
    cp -r ${SCRIPT_DIR}/../../workloads/AI/BERT/pytorch/training/* ${WORK_DIR}
    bench_script_dir=${WORK_DIR}
    date4log=""
    #$(date '+%Y%m%d_%H%M%S')

    if [ "$DOCKER" != "0" ]; then
        script2run=./pt_bertlarge_training_container_run.sh
        log_file_prefix=${LOG_DIR}/pt_bertlarge_training_container_run
        script2setup=./pt_bertlarge_training_container_setup.sh
        log_file_setup=${LOG_DIR}/pt_bertlarge_training_container_setup_${date4log}.log
    else
        script2run=./pt_bertlarge_training_run.sh
        log_file_prefix=${LOG_DIR}/pt_bertlarge_training_run
        script2setup=./pt_bertlarge_training_setup.sh
        log_file_setup=${LOG_DIR}/pt_bertlarge_training_setup_${date4log}.log
    fi

    cd $bench_script_dir

    #setup environments
    print_prefix "AI benchmark evaluation - Pytorch Bert Large Training on $device_name"
    echo "### Set up working environment ..."
    if (( ! $dry_run ));then
        VENV_NAME=${VENV_NAME} $script2setup 2>&1 |tee $log_file_setup
    else
        echo "VENV_NAME=${VENV_NAME} ./pt_bertlarge_training_container_setup.sh"
    fi
    ...
    print_prefix "AI benchmark evaluation - Pytorch Bert Large Training DATAVALIDATION on GPU $device_name"
