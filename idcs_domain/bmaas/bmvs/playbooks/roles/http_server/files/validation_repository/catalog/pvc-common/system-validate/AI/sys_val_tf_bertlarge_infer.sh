...
dry_run=${DRYRUN:-0}
DOCKER=${DOCKER:-0}
VENV_NAME=${VENV_NAME:-bench_tf_bertlarge_infer}

if [ "$DOCKER" != "0" ]; then
    LOG_DIR=${WORKSPACE}/bench_tf_bertlarge_infer_container_logs
    WORK_DIR=${WORKSPACE}/bench_tf_bertlarge_infer_container_workdir
else
    LOG_DIR=${WORKSPACE}/bench_tf_bertlarge_infer_logs
    WORK_DIR=${WORKSPACE}/bench_tf_bertlarge_infer_workdir
fi
mkdir -p ${LOG_DIR}
mkdir -p ${WORK_DIR}
...
validate_tf_bertlarge_infer(){
    ...
    if [ $tiles == 1 ]; then
        ...
        reference_data=(450 900 0 1800 0 0 0 3600)
    elif [ $tiles == 2 ]; then
        #reference dat for Max 1550, sys-821
        num_of_card=(1 2 3 4 5 6 7 8)
        reference_data=(1000 2000 0 4000 0 0 0 8000)
    fi
    perf_val=(0 0 0 0 0 0 0 0)

    #running in benchmark folder
    test_id="tf_bertlarge_inference"
    cp -r ${SCRIPT_DIR}/../../workloads/AI/BERT/tensorflow/inference/* ${WORK_DIR}
    bench_script_dir=${WORK_DIR}
    date4log=""
    #$(date '+%Y%m%d_%H%M%S')
    if [ "$DOCKER" != "0" ]; then
        script2run=./tf_bertlarge_inference_container_run.sh
        log_file_prefix=${LOG_DIR}/tf_bertlarge_inference_container_run
	script2setup=./tf_bertlarge_inference_container_setup.sh
	log_file_setup=${LOG_DIR}/tf_bertlarge_inference_container_setup_${date4log}.log
    else
        script2run=./tf_bertlarge_inference_run.sh
        log_file_prefix=${LOG_DIR}/tf_bertlarge_inference_run
	script2setup=./tf_bertlarge_inference_setup.sh
	log_file_setup=${LOG_DIR}/tf_bertlarge_inference_setup_${date4log}.log
    fi
    cd $bench_script_dir

    #setup environments
    print_prefix "AI benchmark evaluation - Tensorflow Bert Large Inference on $device_name"
    ...
    print_prefix "AI benchmark evaluation - Tensorflow Bert Large Inference DATAVALIDATION on GPU $device_name"
