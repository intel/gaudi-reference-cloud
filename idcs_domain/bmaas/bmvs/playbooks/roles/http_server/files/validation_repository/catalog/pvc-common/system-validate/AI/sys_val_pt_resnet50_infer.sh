...
dry_run=${DRYRUN:-0}
DOCKER=${DOCKER:-0}
VENV_NAME=${VENV_NAME:-bench_pt_resnet50_infer}

if [ "$DOCKER" != "0" ]; then
    LOG_DIR=${WORKSPACE}/bench_pt_resnet50_infer_container_logs
    WORK_DIR=${WORKSPACE}/bench_pt_resnet50_infer_container_workdir
else
    LOG_DIR=${WORKSPACE}/bench_pt_resnet50_infer_logs
    WORK_DIR=${WORKSPACE}/bench_pt_resnet50_infer_workdir
fi
mkdir -p ${LOG_DIR}
mkdir -p ${WORK_DIR}
...
validate_ai_resnet50_pt_infer(){
    ...
    if [ $tiles == 1 ]; then
        ...
        reference_data=(20000 40000 0 80000 0 0 0 160000)
    elif [ $tiles == 2 ]; then
        #reference dat for Max 1550
        num_of_card=(1 2 3 4 5 6 7 8)
        reference_data=(53000 106000 0 210000 0 0 0 410000)  
    fi
    perf_val=(0 0 0 0 0 0 0 0)

    #running in benchmark folder
    test_id="pt_resnet50_inference"
    cp -r ${SCRIPT_DIR}/../../workloads/AI/ResNet50_v1.5/pytorch/inference/* ${WORK_DIR}
    bench_script_dir=${WORK_DIR}
    date4log=""
    #$(date '+%Y%m%d_%H%M%S')
    if [ "$DOCKER" != "0" ]; then
        script2run=./pt_resnet50_inference_container_run.sh
        log_file_prefix=${LOG_DIR}/pt_resnet50_inference_container_run
        script2setup=./pt_resnet50_inference_container_setup.sh
        log_file_setup=${LOG_DIR}/pt_resnet50_inference_container_setup_${date4log}.log
    else
        script2run=./pt_resnet50_inference_run.sh
        log_file_prefix=${LOG_DIR}/pt_resnet50_inference_run
        script2setup=./pt_resnet50_inference_setup.sh
        log_file_setup=${LOG_DIR}/pt_resnet50_inference_setup_${date4log}.log
    fi

    cd $bench_script_dir

    #setup environments
    print_prefix "AI benchmark evaluation - Pytorch Resnet50 v1.5 Inference on $device_name"
    ...
    for i in 1 2 4 8; do
        ...
        if (( ! $dry_run )); then
            VENV_NAME=${VENV_NAME} NUMBER_OF_GPU=$i PRECISION=int8 BATCH_SIZE=1024 $script2run 2>&1 |tee $log_file
        else
	    echo "VENV_NAME=${VENV_NAME} NUMBER_OF_GPU=$i PRECISION=int8 BATCH_SIZE=1024 $script2run"
        fi
        perf_val[$( bc -l <<< "$i -1")]=$( grep "Total Throughput" $log_file | awk -F' ' '{ print $3 }' |tr -d '\r\n' )
       fi
    done
    print_prefix "AI benchmark evaluation - Pytorch Resnet50 v1.5 Inference DATAVALIDATION on GPU $device_name"
