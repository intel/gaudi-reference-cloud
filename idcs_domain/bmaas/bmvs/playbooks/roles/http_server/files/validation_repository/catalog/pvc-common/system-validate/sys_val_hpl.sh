...
LOG_DIR=${WORKSPACE}/bench_hpl_logs
WORK_DIR=${WORKSPACE}/bench_hpl_workdir

mkdir -p ${LOG_DIR}
mkdir -p ${WORK_DIR}
cd ${WORK_DIR}
export PATH=$PATH:${ONEAPI_ROOT}/mkl/latest/share/mkl/benchmarks/mp_linpack/:${WORK_DIR}/benchmarks_2024.0/linux/share/mkl/benchmarks/mp_linpack/
...
hpl_magic_1100=12000
hpl_magic_1550=24000

# order with GPU number 1 2 3 4 5 6 7 8, data with 1 2 4 8
hpl_scale_magic_1100_x8=(12000 24000 0 44000 0 0 0 76000)
hpl_scale_magic_1550_x4=(24000 47000 0 89000 0 0 0 0)
hpl_scale_magic_1550_x8=(24000 47000 0 88000 0 0 0 153000)
...
get_hpl_benchmarks(){
    ...
    hplbench_bin="xhpl_intel64_dynamic_gpu"
    if [ ! -e "`which $hplbench_bin`" ]; then
		pushd ${WORK_DIR}
	    echo "Can't find xhpl_intel64_dynamic_gpu binary for execution"
...
validate_hpl_bench_result(){
	...
	printf "DATAVALIDATION HPL benchmark on GPU %d: %-10s: current=%s(GFLOPS), ref=%s, ratio=%.2f%%\n" $gpu $val_result $benchval $benchref $bench_ratio

	# report to a csv file
	csv_file="hpl_test.csv"
	if ! test -f $csv_file; then
		echo "Workload, GPU, Result, Performance(GFLOPS), Reference, Ratio" > $csv_file
	fi
	echo "HPL,${gpu},${val_result},${benchval},${benchref},${bench_ratio}" >> $csv_file

}

validate_hpl_bench(){
    print_prefix "### Running HPL benchmarking on single GPU in order"
    NNODES=1
    NGPUS=1
    NTILES_PER_GPU=$tiles
    GPUMEM_GB=$gpu_memory
    NB=384
    N=$(echo "(sqrt($NNODES*$NGPUS*$NTILES_PER_GPU*($GPUMEM_GB-6)*1024*1024*1024/8)/$NB+1)*$NB"|bc)
    P=1
    Q=1
    create_hpl_dat_file
   ...
   for numa in ${!numa_gpu_map[@]}; do
	 ...
	 if [ "$tiles" == "1" ]; then
         ...
         cmd="I_MPI_FABRICS=shm HPL_HOST_NODE=$numa HPL_DEVICE=$hpl_device mpirun -np 1 numactl -N $numa -m $numa ${hplbench_bin}"
	 if (( $dry_run )); then
             echo $cmd
	 else
...
validate_hpl_scale_bench(){
   print_prefix "Running HPL scalability benchmarking"   
   for i in ${GPUNUM}; do
     if (( $( echo "$num_gpu >= $i" | bc -l ) )); then
	echo "### Evaluating HPL performance on $i GPU"
	...
	if [ "${#numa_gpu_map0[@]}" -ge "$i" ]; then
	    ...
	    create_wrapper_for_mp 
	    cmd="I_MPI_FABRICS=shm mpirun -np 2 bash ./hpl_wrapper.sh ${hpl_device_rank[@]}"
	...
	if (( $dry_run )); then
        ...
        if [ "${val_func}x" == "x" ]; then
            echo "HPL runtime failed: FUNCFAILED"
        else
	    perf=$(grep WC $log_file|awk '{print int($NF)}')
	    perf_val[$( bc -l <<< "$i -1")]=$perf
	    echo "HPL performance evaluated: $perf GFLOPS"
	fi
   ...
   print_prefix "HPL scalability benchmarking with DATAVALIDATION"
   ...
   for i in $( seq 0 7); do
	   ...
	   echo "DATAVALIDATION HPL benchmarking on $((i+1)) GPU (GFLOPS) - $resultstr : current $perf_data, reference ${hpl_scale_magic[$i]}, ratio $ratio%, scalability $scale% "
	   	# report to a csv file
		csv_file="hpl_test_scale.csv"
		if ! test -f $csv_file; then
			echo "Workload, #GPU, Result, Performance(GFLOPS), Reference, Ratio(%), Scale(%)" > $csv_file
		fi
		echo "HPL,$((i+1)),${resultstr},${perf_data},${hpl_scale_magic[$i]},${ratio},${scale}" >> $csv_file
       fi
   done
