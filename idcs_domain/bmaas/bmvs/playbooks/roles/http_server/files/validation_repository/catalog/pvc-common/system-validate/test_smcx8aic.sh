#!/bin/bash
# Example to run on x8 Max1100 system
result_file=test_smcx8aic_report.log
echo "" >> $result_file
...
passnum=(80 16 65 72 15 12 10 4 4)
for i in $(seq 0 8); do
...
#EXPECTED_PASSNUM=80 TEST2RUN=gemm ./test2run.sh
#EXPECTED_PASSNUM=16 TEST2RUN=stream ./test2run.sh
#EXPECTED_PASSNUM=65 TEST2RUN=zepeer ./test2run.sh
#EXPECTED_PASSNUM=72 TEST2RUN=zebandwidth ./test2run.sh
#EXPECTED_PASSNUM=15 TEST2RUN=onecclbench ./test2run.sh
