...
// Based on example https://github.com/deepmap/oapi-codegen/blob/master/examples/authenticated-api/echo/server/jwt_authenticator.go
package auth
...
func NewAuthenticator(v JWSValidator) openapi3filter.AuthenticationFunc {
	return func(ctx context.Context, input *openapi3filter.AuthenticationInput) error {
		return Authenticate(ctx, v, input)
	}
}
...
func Authenticate(ctx context.Context, v JWSValidator, input *openapi3filter.AuthenticationInput) error {

	// If Login request, do not authenticate user
	if input.RequestValidationInput.Request.URL.Path == "/api/v2/login" {
		return nil
	}
