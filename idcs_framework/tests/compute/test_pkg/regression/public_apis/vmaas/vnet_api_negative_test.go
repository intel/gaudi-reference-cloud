...
var _ = Describe("Compute VNET api endpoint(VM negative flow)", Label("compute", "vmaas", "vmaas_vnet", "vmaas_vnet_negative"), Ordered, ContinueOnFailure, func() {
	...
	When("Creating VNET with invalid char length on name", func() {
		It("should fail with valid error...", func() {
			...
			payload := vnetPayload
			payload = strings.Replace(payload, "<<vnet-name>>", "vnet-name-to-validate-the-character-length-for-testing-purpose-attempt1", 1)
			statusCode, responseBody := service_apis.CreateVnetWithCustomizedPayload(vnetEndpoint, token, payload)
