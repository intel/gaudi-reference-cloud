...
var _ = Describe("Compute SSH public key endpoint(BM negative flow)", Label("compute", "bmaas", "compute_ssh_key", "bmaas_ssh_key", "bmaas_ssh_key_negative"), Ordered, ContinueOnFailure, func() {
	JustBeforeEach(func() {
		logInstance.Println("----------------------------------------------------")
	})
	...
	When("Creating an SSH public key using invalid key", func() {
		It("should fail with valid error...", func() {
			logInstance.Println("Attempt to create SSH-public-key creation with invalid key")
