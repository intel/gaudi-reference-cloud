...
var _ = Describe("Compute instance type endpoint(BM negative flow)", Label("compute", "bmaas", "compute_instance_type", "bmaas_instance_type", "bmaas_instance_type_negative"), Ordered, func() {
	JustBeforeEach(func() {
		logInstance.Println("----------------------------------------------------")
	})
