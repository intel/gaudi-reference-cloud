...
func TestRegressionSuite(t *testing.T) {
	...
	RunSpecs(t, "IDC - Validation Operator Business usecases")
}

var _ = SynchronizedBeforeSuite(func() []byte {
	logger.InitializeLogger(12)
	logInstance = logger.GetLogger()

	utils.SuitelevelLogSetup(logInstance)

	logInstance.Println("Running setup on Node 12")
	logInstance.Println("Starting Validation Operator test suite")
