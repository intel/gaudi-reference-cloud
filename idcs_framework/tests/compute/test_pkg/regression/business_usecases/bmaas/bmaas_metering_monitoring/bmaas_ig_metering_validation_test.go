...
var _ = Describe("BMaas Instance group CRUD operation and validating the records in metering service",
	Label("compute", "compute_business_uc", "bm_instance_group_metering_validation_uc"), Ordered, func() {
		var (
			instanceType                   string
		...
		When("Creating instances within cluster or group", func() {
			It("instances should be created within cluster", func() {
				logInstance.Println("Starting the Instance Group Creation flow via API...")
