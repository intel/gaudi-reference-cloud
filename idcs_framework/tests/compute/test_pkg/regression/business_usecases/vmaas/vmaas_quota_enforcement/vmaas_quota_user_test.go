...
var _ = Describe("IDC - VMaaS Quota Enforcement", Label("compute", "compute_bu", "vmaas_quota"), Ordered, func() {
	var (
		instanceType             string
		machineImage             string
	...
	When("intel user", func() {
		It("intel user", func() {
			...
			intelInstanceList = utils.CreateInstanceWithinQuota(intelInstanceEndpoint, intelToken, intelUserQuota, string(instancePayload), instanceType,
				intelSSHKeyName, intelVnet, machineImage, availabilityZone)
			...
			utils.DeleteInstanceWithinQuota(intelInstanceEndpoint, intelToken, intelInstanceList, 2*time.Minute)
	...
	When("standard user", func() {
		It("standard user", func() {
			...
			// delete the instances
			utils.DeleteInstanceWithinQuota(standardInstanceEndpoint, standardToken, standardInstanceList, 2*time.Minute)
	...
	When("premium user", func() {
		It("premium user", func() {
			...
			// delete the instances
			utils.DeleteInstanceWithinQuota(premiumInstanceEndpoint, premiumToken, premiumInstanceList, 2*time.Minute)
