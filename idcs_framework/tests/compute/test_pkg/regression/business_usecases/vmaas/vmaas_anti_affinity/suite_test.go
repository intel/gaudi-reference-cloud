...
var _ = SynchronizedBeforeSuite(func() []byte {
	logger.InitializeLogger(1)
	logInstance = logger.GetLogger()

	utils.SuitelevelLogSetup(logInstance)

	logInstance.Println("Running setup on Node 1")
	logInstance.Println("Starting VMaaS anti affinity validation suite")

	if skipVMCreation == "true" {
		Skip("Skipping the Entire VM flow due to the flag")
	}

	harvesterKubeConfigPath = "../../resources/kubeconfig/qa1-harvester.yaml"
	client.SetUpKubeClient(harvesterKubeConfigPath, testEnv)
...
func TestRegressionSuite(t *testing.T) {
	...
	RunSpecs(t, "IDC - VMaaS anti affinity Business usecases")
}
