...
var (
	...
	negativeCloudAccountId string
	creationSucceeded      bool
	userEmail, userToken   string
)
...
func init() {
	...
	flag.StringVar(&negativeCloudAccountId, "negativeCloudAccountId", "", "")
...
var _ = SynchronizedBeforeSuite(func() []byte {
	logger.InitializeLogger(8)
	logInstance = logger.GetLogger()

	utils.SuitelevelLogSetup(logInstance)

	logInstance.Println("Running setup on Node 8")
	logInstance.Println("Starting VMaaS Node pool negative test suite")
	...
	// Suite setup
	cloudAccount, instanceEndpoint, sshEndpoint, vnetEndpoint, _, _ = utils.SuiteSetup(testEnv, token, cloudaccountUrl, computeUrl, "ACCOUNT_TYPE_INTEL", negativeCloudAccountId)
...
func TestRegressionSuite(t *testing.T) {
	...
	//logger.Init()
	RunSpecs(t, "IDC - VMaaS Node poolBusiness usecases")
}
