...
func GetNamespaceListWithLabel(selector string, kubeFilePath string) (*v1.NamespaceList, error) {
	ctx := context.Background()
	config, err := utils.GetRESTConfig(kubeFilePath)
	if err != nil {
		return nil, err
	}
...
func GetBmhByConsumer(consumerName string, kubeFilePath string) (*baremetalv1alpha1.BareMetalHost, error) {
	config, err := utils.GetRESTConfig(kubeFilePath)
	if err != nil {
		return nil, err
	}
	...
	namespaces, err := GetNamespaceListWithLabel(selector, kubeFilePath)
	if err != nil {
		return nil, err
	}
...
func GetBmhByName(devicename string, kubeFilePath string) (*baremetalv1alpha1.BareMetalHost, error) {
	config, err := utils.GetRESTConfig(kubeFilePath)
	if err != nil {
		return nil, err
	}
...
func CheckBMHState(system string, state string, timeout int, kubeFilePath string) (bool, error) {
	...
	go func() {
		checkState(stateReached, system, state, kubeFilePath)
		<-stateReached
	}()
	select {
...
func checkState(done chan bool, system string, state string, kubeFilePath string) {
	for {
		bmh, err := GetBmhByName(system, kubeFilePath)
		if err != nil {
