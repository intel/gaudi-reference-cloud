...
class User(HttpUser):
    ...
    def on_start(self):
        ...
        self.raw_payload_string = ScaleUtil.read_json_file_as_string("resources/gaudi2-vm-with-load-cloudinit.json")
    ...
    def make_request(self):
        ...
        try:
            ...
            instance_creation = self.http_client.post_method(self.instance_endpoint,self.token, iteration_payload_string, instance_name, None)
            ...
            instance_phase = ScaleUtil.is_instance_ready(self.http_client, creation_time, self.instance_endpoint, self.token, instance_name, None)
            
            # validate the instance and populate the data for SSH
            if instance_phase == "Ready":
                logging.info("Instance phase : %s" % instance_phase)
                response = self.http_client.get_method(self.instance_endpoint, self.token, instance_name, None)
                ScaleUtil.validate_status_code(response, 200)
            ...
            ssh_command = ['ssh', '-J', f'{proxy_user}@{proxy_ip}', '-o', 'StrictHostKeyChecking=no', '-o', 'UserKnownHostsFile=/dev/null', f'{instance_user}@{instance_ip}', 'sudo', '-i', 'sh', '/tmp/run_container.sh', '>', '/tmp/loadoutput.txt']
    ...
    def assert_ssh_output(result):
        ...
        assert "Habana Labs Ltd. Device" in result.stdout
        logging.info("SSH output: %s" % result.stdout)
