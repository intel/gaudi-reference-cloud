...
class User(HttpUser):
    ...
    def make_request1(self):
        self.get_request("<<instance-id-1>>")
    
    @task(1)
    def make_request2(self):
        self.get_request("<<instance-id-2>>")

    def get_request(self, instance_id):
        qc_endpoint =  self.qcBaseUrl +"/"+self.cloudaccount+"/"+instance_id+"/lab?"
