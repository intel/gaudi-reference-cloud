...
class User(HttpUser):
    ...
    def on_start(self):
        ...
        self.raw_payload_string = ScaleUtil.read_json_file_as_string("resources/gaudi2-vm-without-load.json")
    ...
    def make_request(self):
        ...
        try:
            ...
            ssh_command = ['ssh', '-J', f'{proxy_user}@{proxy_ip}', '-o', 'StrictHostKeyChecking=no', '-o', 'UserKnownHostsFile=/dev/null', f'{instance_user}@{instance_ip}', 'lspci -d :1020: -nn']
            result = ScaleUtil.run_ssh_commands(ssh_command)
            User.assert_ssh_output(result)
            time.sleep(60)
