//go:build Functional || Billing || Intel || IntelIntegration || Integration || MeteringTest
// +build Functional Billing Intel IntelIntegration Integration MeteringTest
...
func (suite *BillingAPITestSuite) Test_Intel_Customer_Push_Invalid_Metering_Records_Validate_Usage() {
	...
	create_payload := financials_utils.EnrichMeteringCreatePayload(compute_utils.GetMeteringCreatePayload(),
		uuid.NewString(), resourceId, cloudAccId, previousDate, "vm-spr-sml", "smallvm", "75000")
	...
	usage_err := billing.ValidateUsage(cloudAccId, float64(3.75), financials_utils.GetIntelSmlVmRate(), "vm-spr-sml", userToken)
...
func (suite *BillingAPITestSuite) Test_Intel_Customer_Push_Metering_Records_Validate_Search() {
	userName := utils.Get_UserName("Intel")
	userToken, _ := auth.Get_Azure_Bearer_Token(userName)
	userToken = "Bearer " + userToken
	authToken := "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
	...
	create_payload := financials_utils.EnrichMeteringCreatePayload(compute_utils.GetMeteringCreatePayload(),
		uuid.NewString(), medResourceId, cloudAccId, previousDate, "vm-spr-med", "mediumvm", "75000")
	fmt.Println("create_payload", create_payload)
	metering_api_base_url := base_url + "/v1/meteringrecords"
	response_status, _ := financials.CreateMeteringRecords(metering_api_base_url, authToken, create_payload)
	assert.Equal(suite.T(), response_status, 200, "Failed: Failed to create metering data")
	...
	create_payload = financials_utils.EnrichMeteringCreatePayload(compute_utils.GetMeteringCreatePayload(),
		uuid.NewString(), smlResourceId, cloudAccId, date1, "vm-spr-sml", "smlvm", "75000")
	fmt.Println("create_payload", create_payload)
	response_status, _ = financials.CreateMeteringRecords(metering_api_base_url, authToken, create_payload)
	assert.Equal(suite.T(), response_status, 200, "Failed: Failed to create metering data")
	...
	for _, v := range arr.Array() {
		if cloudAccId == v.Get("cloudAccountId").String() && medResourceId == v.Get("resourceId").String() {
			if v.Get("properties.runningSeconds").String() == "75000" {
...
func (suite *BillingAPITestSuite) Test_Intel_Customer_Push_Metering_Records_Different_Dates() {
	userName := utils.Get_UserName("Intel")
	userToken, _ := auth.Get_Azure_Bearer_Token(userName)
	userToken = "Bearer " + userToken
	authToken := "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
	...
	arr.ForEach(func(key, value gjson.Result) bool {
		...
		if gjson.Get(data, "productType").String() == "vm-spr-med" {
			...
			assert.Equal(suite.T(), float64(30), actualAMount, "Failed: Failed to validate usage amount")
		...
		if gjson.Get(data, "productType").String() == "vm-spr-sml" {
			...
			Amount := gjson.Get(data, "amount").String()
			actualAMount, _ := strconv.ParseFloat(Amount, 64)
			assert.Equal(suite.T(), float64(1.5), actualAMount, "Failed: Failed to validate usage amount")
	...
	assert.Equal(suite.T(), total_amount_from_response, 31.5, "Failed: Failed to validate usage amount")
