//go:build Functional || Billing || Intel || IntelIntegration || Integration || MockUsage
// +build Functional Billing Intel IntelIntegration Integration MockUsage
...
func (suite *BillingAPITestSuite) TestIntelUsageFreeVM() {
	suite.T().Skip()
...
func (suite *BillingAPITestSuite) TestIntelUsageSmallVM() {
	userName := utils.Get_UserName("Intel")
	userToken, _ := auth.Get_Azure_Bearer_Token(userName)
	userToken = "Bearer " + userToken
	authToken := "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
	...
	fmt.Println("Metering Date", previousDate)
	create_payload := financials_utils.EnrichMeteringCreatePayload(compute_utils.GetMeteringCreatePayload(),
		uuid.NewString(), uuid.NewString(), cloudAccId, previousDate, "vm-spr-sml", "smallvm", "75000")
	fmt.Println("create_payload", create_payload)
	metering_api_base_url := base_url + "/v1/meteringrecords"
	response_status, _ := financials.CreateMeteringRecords(metering_api_base_url, authToken, create_payload)
	assert.Equal(suite.T(), response_status, 200, "Failed: Failed to create metering data")
	...
	usage_err := billing.ValidateUsage(cloudAccId, float64(3.75), financials_utils.GetIntelSmlVmRate(), "vm-spr-sml", authToken)
	assert.Equal(suite.T(), usage_err, nil, "Failed to validate usage, validation failed with error : ", usage_err)
...
func (suite *BillingAPITestSuite) TestIntelUsageMediumVM() {
	userName := utils.Get_UserName("Intel")
	userToken, _ := auth.Get_Azure_Bearer_Token(userName)
	userToken = "Bearer " + userToken
	authToken := "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
	...
	fmt.Println("Metering Date", previousDate)
	create_payload := financials_utils.EnrichMeteringCreatePayload(compute_utils.GetMeteringCreatePayload(),
		uuid.NewString(), uuid.NewString(), cloudAccId, previousDate, "vm-spr-med", "mediumvm", "75000")
	fmt.Println("create_payload", create_payload)
	metering_api_base_url := base_url + "/v1/meteringrecords"
	response_status, _ := financials.CreateMeteringRecords(metering_api_base_url, authToken, create_payload)
	assert.Equal(suite.T(), response_status, 200, "Failed: Failed to create metering data")
...
func (suite *BillingAPITestSuite) TestIntelUsageDateRange() {
	...
	usage_err := billing.ValidateUsageDateRange(cloudAccId, previousDate, endDate1, float64(7.5), financials_utils.GetIntelMedVmRate(), "vm-spr-med", authToken)
	assert.Equal(suite.T(), usage_err, nil, "Failed to validate usage, validation failed with error : ", usage_err)
...
func (suite *BillingAPITestSuite) TestIntelUsageBM() {
	userName := utils.Get_UserName("Intel")
	userToken, _ := auth.Get_Azure_Bearer_Token(userName)
	userToken = "Bearer " + userToken
	authToken := "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
	...
	fmt.Println("Metering Date", previousDate)
	create_payload := financials_utils.EnrichMeteringCreatePayload(compute_utils.GetMeteringCreatePayload(),
		uuid.NewString(), uuid.NewString(), cloudAccId, previousDate, "bm-spr", "bmvm", "77500")
	fmt.Println("create_payload", create_payload)
	metering_api_base_url := base_url + "/v1/meteringrecords"
	response_status, _ := financials.CreateMeteringRecords(metering_api_base_url, authToken, create_payload)
	assert.Equal(suite.T(), response_status, 200, "Failed: Failed to create metering data")
	...
	arr.ForEach(func(key, value gjson.Result) bool {
		...
		if gjson.Get(data, "productType").String() == "bm-spr" {
			...
			actualAMount = testsetup.RoundFloat(actualAMount, 0)
			assert.GreaterOrEqual(suite.T(), actualAMount, float64(31), "Failed: Failed to validate usage amount")
	...
	total_amount_from_response := gjson.Get(usage_response_body, "totalAmount").Float()
	total_amount_from_response = testsetup.RoundFloat(total_amount_from_response, 0)
	assert.GreaterOrEqual(suite.T(), total_amount_from_response, float64(31), "Failed: Failed to validate usage amount")
...
func (suite *BillingAPITestSuite) TestIntelUsageGaudi2() {
	userName := utils.Get_UserName("Intel")
	userToken, _ := auth.Get_Azure_Bearer_Token(userName)
	userToken = "Bearer " + userToken
	authToken := "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
	...
	fmt.Println("Metering Date", previousDate)
	create_payload := financials_utils.EnrichMeteringCreatePayload(compute_utils.GetMeteringCreatePayload(),
		uuid.NewString(), uuid.NewString(), cloudAccId, previousDate, "bm-icp-gaudi2", "gaudi2vm", "46442")
	fmt.Println("create_payload", create_payload)
	metering_api_base_url := base_url + "/v1/meteringrecords"
	response_status, _ := financials.CreateMeteringRecords(metering_api_base_url, authToken, create_payload)
	assert.Equal(suite.T(), response_status, 200, "Failed: Failed to create metering data")
	...
	arr.ForEach(func(key, value gjson.Result) bool {
		...
		if gjson.Get(data, "productType").String() == "bm-icp-gaudi2" {
			...
			actualAMount, _ := strconv.ParseFloat(Amount, 64)
			actualAMount = testsetup.RoundFloat(actualAMount, 0)
			assert.GreaterOrEqual(suite.T(), actualAMount, float64(87), "Failed: Failed to validate usage amount")
	...
	total_amount_from_response := gjson.Get(usage_response_body, "totalAmount").Float()
	total_amount_from_response = testsetup.RoundFloat(total_amount_from_response, 0)
	assert.GreaterOrEqual(suite.T(), total_amount_from_response, float64(87), "Failed: Failed to validate usage amount")
...
func (suite *BillingAPITestSuite) TestIntelUsageLargeVM() {
	userName := utils.Get_UserName("Intel")
	userToken, _ := auth.Get_Azure_Bearer_Token(userName)
	userToken = "Bearer " + userToken
	authToken := "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
...
// func (suite *BillingAPITestSuite) TestIntel_Usage_multi_region() {
// 	userName := utils.Get_UserName("Intel")
// 	userToken, _ := auth.Get_Azure_Bearer_Token(userName)
// 	userToken = "Bearer " + userToken
// 	authToken := "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
...
// 		uuid.NewString(), uuid.NewString(), cloudAccId, previousDate, "vm-spr-sml", "smallvm", "200000", "us-dev-1a", "harvester1", "false", "us-dev-1", "ComputeAsAService")
...
// 	assert.Equal(suite.T(), 200, create_response_status, "Failed: Created Paid instances after using all credits")
