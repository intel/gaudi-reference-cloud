...
func (suite *BillingAPITestSuite) Test_Standard_Customer_Push_Invalid_Metering_Records_Validate_Usage() {
...
func (suite *BillingAPITestSuite) Test_Standard_Customer_Push_Metering_Records_Validate_Search() {
	userName := utils.Get_UserName("Standard")
	userToken, _ := auth.Get_Azure_Bearer_Token(userName)
	userToken = "Bearer " + userToken
	authToken := "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
	...
	create_payload := financials_utils.EnrichMeteringCreatePayload(compute_utils.GetMeteringCreatePayload(),
		uuid.NewString(), medResourceId, cloudAccId, previousDate, "vm-spr-sml", "smallvm", "60000")
	fmt.Println("create_payload", create_payload)
	metering_api_base_url := base_url + "/v1/meteringrecords"
	response_status, _ := financials.CreateMeteringRecords(metering_api_base_url, authToken, create_payload)
	assert.Equal(suite.T(), response_status, 200, "Failed: Failed to create metering data")
	...
	for _, v := range arr.Array() {
		if cloudAccId == v.Get("cloudAccountId").String() && medResourceId == v.Get("resourceId").String() {
			if v.Get("properties.runningSeconds").String() == "60000" {
	...
	usage_err := billing.ValidateUsageDateRange(cloudAccId, previousDate, endDate1, float64(7.5), float64(0.0075), "vm-spr-sml", authToken)
	assert.Equal(suite.T(), usage_err, nil, "Failed to validate usage, validation failed with error : ", usage_err)

	usage_err = billing.ValidateUsageDateRange(cloudAccId, endDate1, endDate2, float64(0), float64(0), "vm-spr-sml", authToken)
	assert.NotEqual(suite.T(), usage_err, nil, "Failed to validate usage, validation failed with error : ", usage_err)

	usage_err = billing.ValidateUsage(cloudAccId, float64(11.25), float64(0.0075), "vm-spr-sml", authToken)
	assert.Equal(suite.T(), usage_err, nil, "Failed to validate usage, validation failed with error : ", usage_err)
...
func (suite *BillingAPITestSuite) Test_Standard_Customer_Push_Metering_Records_Different_Dates() {
	userName := utils.Get_UserName("Standard")
	userToken, _ := auth.Get_Azure_Bearer_Token(userName)
	userToken = "Bearer " + userToken
	authToken := "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
	...
	create_payload = financials_utils.EnrichMeteringCreatePayload(compute_utils.GetMeteringCreatePayload(),
		uuid.NewString(), medResourceId, cloudAccId, date1, "bm-spr", "bmvm", "74627")
	fmt.Println("create_payload", create_payload)
