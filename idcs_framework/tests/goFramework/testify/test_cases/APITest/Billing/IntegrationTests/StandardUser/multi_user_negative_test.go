//go:build Functional || Billing || Standard || StandardIntegration || Integration || MultiUserOPA
// +build Functional Billing Standard StandardIntegration Integration MultiUserOPA
...
func (suite *BillingAPITestSuite) Test_Intel_Verify_Member_Cannot_SendInvite() {
	...
	assert.Equal(suite.T(), "3", errmsgCode, "Test Failed : Code in error message is not correct")
	assert.Equal(suite.T(), "only intel, premium and enterprise accounts are supported", errmsg, "Test Failed : Code in error message is not correct")
	...
	// Verify Member is not able to check invoices of admin
	url := base_url + "/v1/billing/invoices"
	logger.Log.Info("Verify Member is not able to check invoices of admin")
	code, body = financials.GetInvoice(url, membertoken, cloudAccId)
	logger.Logf.Infof("GetInvoice response code  :  %d ", code)
	logger.Logf.Infof("GetInvoice response body :  %s ", body)
	assert.Equal(suite.T(), 403, code, "Test Failed : Did not receive 403 status code")
	assert.NotEqual(suite.T(), 200, code, "Test Failed : Should not receive 200 status code")
	errmsgCode = gjson.Get(body, "code").String()
	assert.Equal(suite.T(), "7", errmsgCode, "Test Failed : Code in error message is not correct")

	// Verify Member is not able to check billing options of admin
	url = base_url + "/v1/billing/invoices"
	logger.Log.Info("Verify Member is not able to check billing options of admin")
	code, body = financials.GetBillingOptions(base_url, membertoken, cloudAccId)
	logger.Logf.Infof("GetBillingOptions response code  :  %d ", code)
	logger.Logf.Infof("GetBillingOptions response body :  %s ", body)
	assert.Equal(suite.T(), 403, code, "Test Failed : Did not receive 403 status code")
	assert.NotEqual(suite.T(), 200, code, "Test Failed : Should not receive 200 status code")
	errmsgCode = gjson.Get(body, "code").String()
	assert.Equal(suite.T(), "7", errmsgCode, "Test Failed : Code in error message is not correct")
