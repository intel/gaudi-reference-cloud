//go:build Functional || MaaS || StandardIntegration
// +build Functional MaaS StandardIntegration
...
func (suite *BillingAPITestSuite) Test_Standard_Create_MaaS_Usage_Record_And_Search() {
...
func (suite *BillingAPITestSuite) Test_Standard_Create_MaaS_Usage_Record_Validate_Usage_For_Text() {
	userName := utils.Get_UserName("Standard")
	userToken, _ := auth.Get_Azure_Bearer_Token(userName)
	userToken = "Bearer " + userToken
	authToken := "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
	...
	coupon_err := billing.Create_Redeem_Coupon("Standard", int64(5), int64(2), cloudAccId)
	...
	estimateUsage := financials_utils.GetStandardTextRate() * float64(10)
	estimateRemaining := float64(5) - float64(estimateUsage)

	logger.Logf.Info("Text Rate for intel : ", financials_utils.GetStandardTextRate())

	usage_err := billing.ValidateUsage(cloudAccId, float64(estimateUsage), financials_utils.GetStandardTextRate(), "modelasaservice-text-processing", authToken)
...
func (suite *BillingAPITestSuite) Test_Standard_Create_MaaS_Usage_Record_Validate_Usage_For_Image() {
	userName := utils.Get_UserName("Standard")
	userToken, _ := auth.Get_Azure_Bearer_Token(userName)
	userToken = "Bearer " + userToken
	authToken := "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
	...
	usage_err := billing.ValidateUsage(cloudAccId, float64(estimateUsage), financials_utils.GetStandardTextRate(), "modelasaservice-image-processing", authToken)
	assert.Equal(suite.T(), usage_err, nil, "Failed to validate usage, validation failed with error : ", usage_err)
...
func (suite *BillingAPITestSuite) Test_Standard_Create_MaaS_Usage_Record_Validate_Usage_For_Image_And_Text() {
	userName := utils.Get_UserName("Standard")
	userToken, _ := auth.Get_Azure_Bearer_Token(userName)
	userToken = "Bearer " + userToken
	authToken := "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
	...
	estimatetextUsage := financials_utils.GetStandardTextRate() * float64(200)
	estimateimageUsage := financials_utils.GetStandardImageRate() * float64(100)
	totalUsage := estimatetextUsage + estimateimageUsage
	estimateRemaining := float64(15) - float64(totalUsage)

	usage_err := billing.ValidateUsage(cloudAccId, float64(estimatetextUsage), financials_utils.GetStandardTextRate(), "modelasaservice-text-processing", authToken)
	assert.Equal(suite.T(), usage_err, nil, "Failed to validate usage, validation failed with error : ", usage_err)

	usage_err = billing.ValidateUsage(cloudAccId, float64(estimateimageUsage), financials_utils.GetStandardImageRate(), "modelasaservice-image-processing", authToken)
	assert.Equal(suite.T(), usage_err, nil, "Failed to validate usage, validation failed with error : ", usage_err)
...
func (suite *BillingAPITestSuite) Test_Standard_User_Uses_All_Credits_And_Tries_to_Launch_instance() {
	userName := utils.Get_UserName("Standard")
	userToken, _ := auth.Get_Azure_Bearer_Token(userName)
	userToken = "Bearer " + userToken
	authToken := "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
...
func (suite *BillingAPITestSuite) Test_Standard_User_Uses_Less_Credits_And_Tries_to_Launch_instance() {
	userName := utils.Get_UserName("Standard")
	userToken, _ := auth.Get_Azure_Bearer_Token(userName)
	userToken = "Bearer " + userToken
	authToken := "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
	...
	coupon_err := billing.Create_Redeem_Coupon("Standard", int64(25), int64(2), cloudAccId)
	assert.Equal(suite.T(), coupon_err, nil, "Failed to create coupon, failed with error : ", coupon_err)
...
func (suite *BillingAPITestSuite) Test_Standard_User_Uses_Adds_Credits_After_100_percent_And_Tries_to_Launch_instance() {
	userName := utils.Get_UserName("Standard")
	userToken, _ := auth.Get_Azure_Bearer_Token(userName)
	userToken = "Bearer " + userToken
	authToken := "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
