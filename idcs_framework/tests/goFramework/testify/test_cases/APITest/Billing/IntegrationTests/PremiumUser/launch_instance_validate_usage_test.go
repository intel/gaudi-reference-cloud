//go:build Functional || Billing || Premium || PremiumIntegration || Integration || InstanceLaunch
// +build Functional Billing Premium PremiumIntegration Integration InstanceLaunch
...
func (suite *BillingAPITestSuite) Test_Premium_Usage_Multiple_VM() {
	...
	create_payload = financials_utils.EnrichMeteringCreatePayload(compute_utils.GetMeteringCreatePayload(),
		uuid.NewString(), uuid.NewString(), cloudAccId, previousDate, "vm-spr-med", "medvm", "30000")
	fmt.Println("create_payload", create_payload)
	metering_api_base_url = baseUrl + "/v1/meteringrecords"
	response_status, _ = financials.CreateMeteringRecords(metering_api_base_url, authToken, create_payload)
	assert.Equal(suite.T(), response_status, 200, "Failed: Failed to create metering data")
	...
	usage_err = billing.ValidateUsage(cloudAccId, float64(15), float64(0.03), "vm-spr-lrg", authToken)
	assert.Equal(suite.T(), usage_err, nil, "Failed to validate usage, validation failed with error : ", usage_err)
...
func (suite *BillingAPITestSuite) Test_Premium_Validate_all_Usages() {
	userName := utils.Get_UserName("Premium")
	userToken, _ := auth.Get_Azure_Bearer_Token(userName)
	userToken = "Bearer " + userToken
	authToken := "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
...
func (suite *BillingAPITestSuite) Test_Premium_User_Launch_Small_Instance_Validate_Usage() {
	userName := utils.Get_UserName("Premium")
	userToken, _ := auth.Get_Azure_Bearer_Token(userName)
	userToken = "Bearer " + userToken
	authToken := "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
...
func (suite *BillingAPITestSuite) Test_Premium_User1_Launch_Small_Instance_Validate_Usage() {
	userName := utils.Get_UserName("Premium")
	userToken, _ := auth.Get_Azure_Bearer_Token(userName)
	userToken = "Bearer " + userToken
	authToken := "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
...
func (suite *BillingAPITestSuite) Test_Premium_Usage_Multiple_VM_Mock() {
	userName := utils.Get_UserName("Premium")
	userToken, _ := auth.Get_Azure_Bearer_Token(userName)
	userToken = "Bearer " + userToken
	authToken := "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
...
func (suite *BillingAPITestSuite) Test_Premium_User_Launch_Medium_Instance_Validate_Usage() {
	userName := utils.Get_UserName("Premium")
	userToken, _ := auth.Get_Azure_Bearer_Token(userName)
	userToken = "Bearer " + userToken
	authToken := "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
	...
	usage_err := billing.ValidateUsageNotZero(cloudAccId, float64(0.015), "vm-spr-med", authToken)
	assert.Equal(suite.T(), usage_err, nil, "Failed to validate usages, validation failed with error : ", usage_err)
	time.Sleep(5 * time.Minute)
	credits_err := billing.ValidateCreditsNonZeroDepletion(cloudAccId, 20, authToken)
	assert.Equal(suite.T(), credits_err, nil, "Failed to validate credits, validation failed with error : ", credits_err)
...
func (suite *BillingAPITestSuite) Test_Premium_User_Launch_Large_Instance_Validate_Usage() {
	userName := utils.Get_UserName("Premium")
	userToken, _ := auth.Get_Azure_Bearer_Token(userName)
	userToken = "Bearer " + userToken
	authToken := "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
	...
	usage_err := billing.ValidateUsageNotZero(cloudAccId, float64(0.03), "vm-spr-lrg", authToken)
	assert.Equal(suite.T(), usage_err, nil, "Failed to validate usages, validation failed with error : ", usage_err)
	time.Sleep(5 * time.Minute)
	credits_err := billing.ValidateCreditsNonZeroDepletion(cloudAccId, 20, authToken)
	assert.Equal(suite.T(), credits_err, nil, "Failed to validate credits, validation failed with error : ", credits_err)
...
func (suite *BillingAPITestSuite) Test_Premium_User_Launch_BM_Instance_Validate_Usage() {
	userName := utils.Get_UserName("Premium")
	userToken, _ := auth.Get_Azure_Bearer_Token(userName)
	userToken = "Bearer " + userToken
	authToken := "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
...
func (suite *BillingAPITestSuite) Test_Premium_User_Launch_Gaudi_Instance_Validate_Usage() {
	userName := utils.Get_UserName("Premium")
	userToken, _ := auth.Get_Azure_Bearer_Token(userName)
	userToken = "Bearer " + userToken
	authToken := "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
	...
	usage_err := billing.ValidateUsageNotZero(cloudAccId, float64(0.1737), "bm-icp-gaudi2", authToken)
	assert.Equal(suite.T(), usage_err, nil, "Failed to validate usages, validation failed with error : ", usage_err)
	time.Sleep(5 * time.Minute)
	credits_err := billing.ValidateCreditsNonZeroDepletion(cloudAccId, 20, authToken)
	assert.Equal(suite.T(), credits_err, nil, "Failed to validate credits, validation failed with error : ", credits_err)
