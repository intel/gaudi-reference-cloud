...
func (suite *BillingAPITestSuite) TestDisableCouponandValidate() {
	...
	couponData = gjson.Get(getdata, "coupons")
	for _, val := range couponData.Array() {
		assert.NotEqual(suite.T(), gjson.Get(val.String(), "disabled").String(), "null", "Failed: Create Coupon Failed to validate disabled")
	}
}
