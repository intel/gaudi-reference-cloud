...
var _ = Describe("VM Instance", Ordered, Label("Compute-VMaaS-E2E"), func() {
	...
	It("Create cloud account", func() {
		...
		// cloud_account_created = "623310341677"
		// place_holder_map["cloud_account_id"] = cloud_account_created
	...
	It("Create vnet with name", func() {
		...
		vnet_name_su := "Autovnet-" + utils.GenerateSSHKeyName(4)
		vnet_payload := compute_utils.EnrichVnetPayload(compute_utils.GetVnetPayload(), vnet_name_su)
		// hit the api
		...
		Expect(vnet_name_su).To(Equal(vnet_created), "Failed to create Vnet, response validation failed")
		//Expect(strings.Contains(vnet_creation_body, `"name":"test-vnet"`)).To(BeTrue(), "Failed to create Vnet, response validation failed")
		//vnet_created = gjson.Get(vnet_creation_body, "metadata.name").String()
		//vnet_created = "standard-vnet-1"
		fmt.Println("vnet_created", vnet_created)
	})
