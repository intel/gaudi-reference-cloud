...
var place_holder_map2 = make(map[string]string)
...
var _ = Describe("VM Instance", Ordered, Label("Compute-VMaaS-E2E"), func() {
	...
	It("Create cloud account", func() {
		//cloud account creation code goes here
		...
		place_holder_map2["cloud_account_id"] = cloud_account_created
		place_holder_map2["cloud_account_type"] = cloudaccount_type
		fmt.Println("cloudAccount_id", cloud_account_created)
		// cloud_account_created = "064129735349"
		// place_holder_map2["cloud_account_id"] = cloud_account_created
		fmt.Println("cloudAccount_id", cloud_account_created)
	})
	...
	It("Create vm instance", func() {
		...
		instance_payload := compute_utils.EnrichInstancePayload(compute_utils.GetInstancePayload(), vm_name_iu, instanceType, "ubuntu-2204-jammy-v20230122", ssh_publickey_name_created, vnet_created)
		fmt.Println("instance_payload", instance_payload)
		place_holder_map2["instance_type"] = instanceType
		// hit the api
		create_response_status_iu, create_response_body_iu = frisby.CreateInstance(instance_endpoint, token, instance_payload)
		...
		place_holder_map2["instance_creation_time"] = strconv.FormatInt(time.Now().UnixMilli(), 10)
	})
	It("Get the created instance and validate", func() {
		...
		place_holder_map2["resource_id"] = instance_id_created
		place_holder_map2["machine_ip"] = gjson.Get(response_body, "status.interfaces[0].addresses[0]").String()
		fmt.Println("IP Address is :" + place_holder_map2["machine_ip"])
	})
	...
	// 	inventory_generated := compute_utils.EnrichInventoryData(inventory_raw_data, proxyIp, place_holder_map2["machine_ip"], "~/.ssh/id_rsa")
	...
	It("Delete the created instance", func() {
		...
		place_holder_map2["instance_deletion_time"] = strconv.FormatInt(time.Now().UnixMilli(), 10)
	})
	...
	It("Get the created instance details from poduct catalog and validate", func() {
		// product_id := place_holder_map2["resource_id"]
	...
	It("Get Metering data related to product and validate", func() {
		search_payload := financials_utils.EnrichMmeteringSearchPayload(compute_utils.GetMeteringSearchPayload(),
			place_holder_map2["resource_id"], place_holder_map2["cloud_account_id"])
		...
		Expect(response_status).To(Equal(200), "Failed to retrieve Product Details")
		Expect(place_holder_map2["cloud_account_id"]).To(Equal(metering_record_cloudAccountId), "Validation failed on cloud account id retrieval")
		Expect(place_holder_map2["resource_id"]).To(Equal(metering_record_resourceId), "Validation failed on resource id retrieval")
		Expect(meta_data_map2["instanceType"]).To(Equal(metering_record_instanceType), "Validation failed on instance retrieval")
		place_holder_map2["transactionId"] = metering_record_transactionId
		place_holder_map2["reported"] = metering_record_reported
	})

	It("Validate billing account is Created for Intel User", func() {
		response_status, responseBody := financials.GetAriaAccountDetailsAllForClientId(place_holder_map2["cloud_account_id"], ariaclientId_iu, ariaAuth)
		fmt.Println("ariaclientId_iu", ariaclientId_iu)
		fmt.Println("ariaAuth", ariaAuth)
		fmt.Println(place_holder_map2["cloud_account_id"])
	...
	It("Delete cloud account", func() {
		...
		delete_Cacc, _ := financials.DeleteCloudAccountById(url, token, place_holder_map2["cloud_account_id"])
	...
	It("Validating deletion of cloud account by retrieving cloud account details", func() {
		...
		url := base_url + "/v1/cloudaccounts"
		resp_body, _ := financials.GetCloudAccountById(url, token, place_holder_map2["cloud_account_id"])
