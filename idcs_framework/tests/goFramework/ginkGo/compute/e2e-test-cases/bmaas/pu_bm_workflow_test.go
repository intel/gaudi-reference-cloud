...
var _ = Describe("BMaaS E2E flow for Premium user", Ordered, Label("large"), func() {
	...
	var place_holder_map1 = make(map[string]string)
	var meta_data_map1 = make(map[string]string)
	var ariaclientIdPremiumBM string
	var ariaAuthPremiumBM string
	...
	BeforeAll(func() {
		...
		ariaclientIdPremiumBM = financials_utils.GetAriaClientNo()
		ariaAuthPremiumBM = financials_utils.GetariaAuthKey()
		fmt.Println("ariaAuthPremiumBM", ariaAuthPremiumBM)
		fmt.Println("ariaclientIdPremiumBM", ariaclientIdPremiumBM)
	})
	...
	XIt("Create cloud account enroll", func() {
		...
		place_holder_map1["cloud_account_id"] = cloud_account_created
		place_holder_map1["cloud_account_type"] = cloudaccount_type
	...
	It("Create cloud account", func() {
		...
		cloudaccount_payload := fmt.Sprintf(`{"name":"%s","owner":"%s","tid":"%s","oid":"%s","type":"ACCOUNT_TYPE_INTEL"}`, "premium-cloudaccount-bm", username, tid, oid)
	...
	It("Create BM instance", func() {
		...
		place_holder_map1["instance_type"] = instanceType
		...
		place_holder_map1["resource_id"] = instance_id_created
		place_holder_map1["instance_creation_time"] = strconv.FormatInt(time.Now().UnixMilli(), 10)
	})
	...
	It("Get BMH device by resource ID", func() {
		...
		place_holder_map1["device_name"] = response_bmh.ObjectMeta.Name
	})

	It("Validate BMH device is provisoned", func() {
		log.Printf("Starting the BMH Device Validation via Kube...")
		succeded, err := kube.CheckBMHState(place_holder_map1["device_name"], "provisioned", 900)
	...
	It("Validate instance is ready and get its details", func() {
		...
		place_holder_map1["machine_ip"] = gjson.Get(response_body, "status.interfaces.0.addresses.0").String()
		place_holder_map1["proxy_ip"] = gjson.Get(response_body, "status.sshProxy.proxyAddress").String()
		place_holder_map1["proxy_user"] = gjson.Get(response_body, "status.sshProxy.proxyUser").String()
		log.Printf("Instance IP Address is: " + place_holder_map1["machine_ip"])
	})
	...
	It("SSH into the BM instance", func() {
		...
		inventory_generated := compute_utils.EnrichInventoryData(inventory_raw_data, place_holder_map1["proxy_ip"], place_holder_map1["proxy_user"], place_holder_map1["machine_ip"], "~/.ssh/id_rsa")
	...
	It("Validate instance deprovision", func() {
		...
		succeded, err := kube.CheckBMHState(place_holder_map1["device_name"], "available", 900)
		Expect(err).Error().ShouldNot(HaveOccurred(), "Timeout reached; "+
			"unable to reach state within expected time")
		Expect(succeded).To(Equal(true))
	})
	...
	It("Get the created instance details from product catalog and validate", func() {
		...
		// product_id := place_holder_map1["resource_id"]
		...
		meta_data_map1["Highlight"] = structResponse.Products[0].Metadata.Highlight
		meta_data_map1["disks.size"] = structResponse.Products[0].Metadata.Disks
		meta_data_map1["instanceType"] = structResponse.Products[0].Metadata.InstanceType
		meta_data_map1["memory.size"] = structResponse.Products[0].Metadata.Memory
		meta_data_map1["region"] = structResponse.Products[0].Metadata.Region
		fmt.Println("meta_data_map1", meta_data_map1)
	})
	...
	It("Get Metering data related to product and validate", func() {
		...
		search_payload := financials_utils.EnrichMmeteringSearchPayload(compute_utils.GetMeteringSearchPayload(),
			place_holder_map1["resource_id"], place_holder_map1["cloud_account_id"])
		...
		Expect(response_status).To(Equal(200), "Failed to retrieve Product Details")
		Expect(place_holder_map1["cloud_account_id"]).To(Equal(metering_record_cloudAccountId), "Validation failed on cloud account id retrieval")
		Expect(place_holder_map1["resource_id"]).To(Equal(metering_record_resourceId), "Validation failed on resource id retrieval")
		Expect(meta_data_map1["instanceType"]).To(Equal(metering_record_instanceType), "Validation failed on instance retrieval")
		place_holder_map1["transactionId"] = metering_record_transactionId
		place_holder_map1["reported"] = metering_record_reported
	})

	XIt("Validate billing account is Created for Premium User", func() {
		log.Printf("Starting the Billing Account Validation via API...")
		client_acct_id := "idc." + cloud_account_created
		response_status, responseBody := financials.GetAriaAccountDetailsAllForClientId(place_holder_map1["cloud_account_id"], ariaclientIdPremiumBM, ariaAuthPremiumBM)
	...
	It("Delete cloud account", func() {
		...
		delete_Cacc, _ := financials.DeleteCloudAccountById(url, token, place_holder_map1["cloud_account_id"])
		Expect(delete_Cacc).To(Equal(200), "Failed to delete Cloud Account")

		// validate the deletion
		get_response_status, _ := financials.GetCloudAccountById(url, token, place_holder_map1["cloud_account_id"])
