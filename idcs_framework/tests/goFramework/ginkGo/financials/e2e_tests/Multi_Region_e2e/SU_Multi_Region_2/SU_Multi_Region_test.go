...
var _ = Describe("Check Standard User Rates Multi Region", Ordered, Label("Multi-Region-E2E"), func() {
	...
	It("Validate Premium cloudAccount", func() {
		...
		Expect(cloudAccountType).To(Equal("ACCOUNT_TYPE_STANDARD"), "Failed to retrieve CloudAccount Type")
		cloudAccIdid = gjson.Get(body, "id").String()
		Expect(cloudAccIdid).NotTo(BeNil(), "Failed to retrieve CloudAccount ID")
	})

	It("Validate Usages region 2", func() {
