//go:build iu_vm || All || pu_su_iu || In5
// +build iu_vm All pu_su_iu In5
...
var _ = Describe("VM Instance", Ordered, Label("Compute-VMaaS-E2E"), func() {
	...
	It("Create paid vm instance when iu user have zero cloud credits", func() {
		...
		instance_payload := compute_utils.EnrichInstancePayload(compute_utils.GetInstancePayload(), vm_name_iu, "vm-spr-lrg", "ubuntu-2204-jammy-v20230122", ssh_publickey_name_created, vnet_created)
		fmt.Println("instance_payload", instance_payload)
		place_holder_map["instance_type"] = "vm-spr-lrg"
		// hit the api
	...
	It("Get the created instance details from poduct catalog and validate", func() {
		...
		Expect(strings.Contains(response_body, `"name":"vm-spr-lrg"`)).To(BeTrue(), "Validation failed on instance retrieval")
