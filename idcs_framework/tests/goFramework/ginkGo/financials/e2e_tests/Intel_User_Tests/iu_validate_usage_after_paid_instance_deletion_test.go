//go:build iu_vm || All || pu_su_iu || In10
// +build iu_vm All pu_su_iu In10
...
var _ = Describe("VM Instance", Ordered, Label("Compute-VMaaS-E2E"), func() {
	...
	It("Validate Usage calculation for the paid product and validate credits depletion", func() {
		...
		Expect(usage1).Should(BeNumerically(">", float64(zeroamt)), "Failed: Validating Usage failed failed should be greater than zero")
	...
	It("Delete paid instance and validate usage is not increasing", func() {
		...
		Expect(runSeconds).To(Equal(newrunSeconds), "Failed: Validating Usage failed for intel user for paid product, runSeconds did not match")
		...
		Expect(freerunSeconds).Should(BeNumerically("<", newfreerunSeconds), "Failed: Validating Usage failed for intel user for paid product, runSeconds did not match")
		Expect(freeamount).To(Equal(newfreeamount), "Failed: Validating Usage failed for intel user for paid product, runSeconds did not match")
