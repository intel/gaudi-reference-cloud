//go:build iu_vm || All || pu_su_iu || iu_free || In6
// +build iu_vm All pu_su_iu iu_free In6
...
var _ = Describe("VM Instance", Ordered, Label("Compute-VMaaS-E2E"), func() {
	...
	It("Validate Usage calculation for the paid product and validate credits depletion", func() {
		...
		Expect(usedAmount).To(Equal(float64(1)), "Failed: Validating Used Credits failed, used Credits didnt match")
