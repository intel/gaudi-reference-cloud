//go:build iu_vm || All || pu_su_iu || iu_free || In3
// +build iu_vm All pu_su_iu iu_free In3
...
var _ = Describe("VM Instance", Ordered, Label("Compute-VMaaS-E2E"), func() {
	...
	It("Validate Usage calculation for the free product", func() {
		...
		amt := 0
		Expect(usedAmount).To(Equal(float64(amt)), "Failed: Validating Used Credits failed failed should be greater than zero")
