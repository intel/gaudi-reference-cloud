//go:build iu_vm || All || pu_su_iu || iu_free || In7
// +build iu_vm All pu_su_iu iu_free In7
...
var _ = Describe("VM Instance", Ordered, Label("Compute-VMaaS-E2E"), func() {
	...
	BeforeAll(func() {
		...
		fmt.Println("Admin Token generated : ", token)
	...
	It("Validate Usage calculation for the paid product and validate credits depletion", func() {
		...
		zeroamt := 0
		Expect(remainingAmount).To(Equal(float64(zeroamt)), "Estimated usage from Credit page is not equal to Credit from CLoud credits")
		Expect(usedAmount).To(Equal(float64(1)), "Failed: Validating Used Credits failed failed should be greater than zero")
	...
	It("Create paid vm instance when cloud account has no credits", func() {
		fmt.Println("Starting the Instance Creation via API...")
		// form the endpoint and payload
		instance_endpoint := compute_url + "/v1/cloudaccounts/" + cloud_account_created + "/instances"
		vm_name_iu := "autovm-" + utils.GenerateSSHKeyName(4)
		...
		Expect(create_response_status).To(Equal(403), "Paid instance created after Intel user used all the credits")
	})
