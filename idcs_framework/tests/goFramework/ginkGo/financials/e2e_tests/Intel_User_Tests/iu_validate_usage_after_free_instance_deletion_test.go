//go:build iu_vm || All || pu_su_iu || In9
// +build iu_vm All pu_su_iu In9
...
var _ = Describe("VM Instance", Ordered, Label("Compute-VMaaS-E2E"), func() {
	...
	It("Create vm instance", func() {
		...
		place_holder_map["instance_name"] = vm_name_iu
	})
	...
	It("Get the created instance details from poduct catalog and validate", func() {
		...
		place_holder_map["Highlight"] = structResponse.Products[0].Metadata.Highlight
		place_holder_map["disks.size"] = structResponse.Products[0].Metadata.Disks
		place_holder_map["instanceType"] = structResponse.Products[0].Metadata.InstanceType
		place_holder_map["memory.size"] = structResponse.Products[0].Metadata.Memory
		place_holder_map["region"] = structResponse.Products[0].Metadata.Region
		fmt.Println("place_holder_map", place_holder_map)
	})
	...
	It("Get Metering data related to product and validate", func() {
		...
		Expect(place_holder_map["instanceType"]).To(Equal(metering_record_instanceType), "Validation failed on instance retrieval")
	...
	// 	now := time.Now().UTC()
	// 	previousDate := now.AddDate(0, 0, 0).Format("2006-01-02T15:04:05.999999Z")
	...
	// 		uuid.NewString(), place_holder_map["resource_id1"], place_holder_map["cloud_account_id"], previousDate,
	// 		place_holder_map["instance_type"], place_holder_map["paid_instance_name"], "30000")
	...
	It("Validate Usage calculation for the paid product and validate credits depletion", func() {
		...
		Expect(usage1).Should(BeNumerically(">", float64(zeroamt)), "Failed: Validating Calculated Used Credits failed should be greater than zero")
		...
		Expect(usedAmount).Should(BeNumerically(">", float64(zeroamt)), "Failed: Validating Used Credits failed failed should be greater than zero")
		//Expect(usage1).To(Equal(usedAmount), "Estimated usage from Credit page is not equal to Credit from CLoud credits")
		Expect(usage1).Should(BeNumerically(">", float64(zeroamt)), "Failed: Validating Usage failed should be greater than zero")

	})

	It("Delete free instance and validate usage is not increasing", func() {
		token = "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
		userToken, _ = auth.Get_Azure_Bearer_Token(userName)
		userToken = "Bearer " + userToken
		instance_endpoint := compute_url + "/v1/cloudaccounts/" + cloud_account_created + "/instances"
		...
		runSeconds := testsetup.GetProductRunSec(userName, "vm-spr-sml")
		amount := testsetup.GetProductUsage(userName, "vm-spr-sml")
		paidrunSeconds := testsetup.GetProductRunSec(userName, "vm-spr-med")
		paidamount := testsetup.GetProductUsage(userName, "vm-spr-med")
		...
		Expect(runSeconds).To(Equal(newrunSeconds), "Failed: Validating Usage failed for intel user for free product, runSeconds did not match")
		Expect(amount).To(Equal(newamount), "Failed: Validating Usage failed for intel user for free product, runSeconds did not match")
		...
		newpaidrunSeconds := testsetup.GetProductRunSec(userName, "vm-spr-med")
		newpaidamount := testsetup.GetProductUsage(userName, "vm-spr-med")
		Expect(paidrunSeconds).Should(BeNumerically("<", newpaidrunSeconds), "Failed: Validating Usage failed for intel user for free product, runSeconds did not match")
		Expect(paidamount).Should(BeNumerically("<", newpaidamount), "Failed: Validating Usage failed for intel user for free product, runSeconds did not match")
		fmt.Println("Intel user usage data", usage2)
		Expect(usage4).Should(BeNumerically(">", usage2), "Failed: Validating Usage failed for intel user for free product, usages did not match")
	})
