//go:build iu_vm_only || All || pu_su_iu
// +build iu_vm_only All pu_su_iu
...
var create_response_status_iu_vm int
var create_response_body_iu_vm string
var place_holder_map_iu_2 = make(map[string]string)
var meta_data_map_iu_2 = make(map[string]string)
var ariaclientId_iu_2 string
...
var _ = Describe("VM Instance", Ordered, Label("Compute-VMaaS-E2E"), func() {
	...
	BeforeAll(func() {
		...
		ariaclientId_iu_2 = financials_utils.GetAriaClientNo()
		...
		fmt.Println("ariaclientId_iu_2", ariaclientId_iu_2)
	})
	...
	It("Create vm instance", func() {
		...
		place_holder_map_iu_2["instance_type"] = "vm-spr-sml"
		// hit the api
		create_response_status_iu_vm, create_response_body_iu_vm = frisby.CreateInstance(instance_endpoint, token, instance_payload)
		VMName_iu := gjson.Get(create_response_body_iu_vm, "metadata.name").String()
		Expect(create_response_status_iu_vm).To(Equal(200), "Failed to create VM instance")
		Expect(vm_name_iu).To(Equal(VMName_iu), "Failed to create VM instance, resposne validation failed")
		place_holder_map_iu_2["instance_creation_time"] = strconv.FormatInt(time.Now().UnixMilli(), 10)
	})
	...
	It("Get the created instance and validate", func() {
		...
		instance_id_created = gjson.Get(create_response_body_iu_vm, "metadata.resourceId").String()
		...
		place_holder_map_iu_2["resource_id"] = instance_id_created
		place_holder_map_iu_2["machine_ip"] = gjson.Get(response_body, "status.interfaces[0].addresses[0]").String()
		fmt.Println("IP Address is :" + place_holder_map_iu_2["machine_ip"])
	})
	...
	// 	inventory_generated := compute_utils.EnrichInventoryData(inventory_raw_data, proxyIp, place_holder_map_iu_2["machine_ip"], "~/.ssh/id_rsa")
	...
	It("Get the created instance details from poduct catalog and validate", func() {
		...
		meta_data_map_iu_2["Highlight"] = structResponse.Products[0].Metadata.Highlight
		meta_data_map_iu_2["disks.size"] = structResponse.Products[0].Metadata.Disks
		meta_data_map_iu_2["instanceType"] = structResponse.Products[0].Metadata.InstanceType
		meta_data_map_iu_2["memory.size"] = structResponse.Products[0].Metadata.Memory
		meta_data_map_iu_2["region"] = structResponse.Products[0].Metadata.Region
		fmt.Println("meta_data_map_iu_2", meta_data_map_iu_2)
	})
	...
	It("Get Metering data related to product and validate", func() {
		search_payload := financials_utils.EnrichMmeteringSearchPayload(compute_utils.GetMeteringSearchPayload(),
			place_holder_map_iu_2["resource_id"], place_holder_map_iu_2["cloud_account_id"])
		...
		Expect(response_status).To(Equal(200), "Failed to retrieve Product Details")
		Expect(place_holder_map_iu_2["cloud_account_id"]).To(Equal(metering_record_cloudAccountId), "Validation failed on cloud account id retrieval")
		Expect(place_holder_map_iu_2["resource_id"]).To(Equal(metering_record_resourceId), "Validation failed on resource id retrieval")
		Expect(meta_data_map_iu_2["instanceType"]).To(Equal(metering_record_instanceType), "Validation failed on instance retrieval")
		place_holder_map_iu_2["transactionId"] = metering_record_transactionId
		place_holder_map_iu_2["reported"] = metering_record_reported
	})

	It("Validate billing account is  not Created for Intel User", func() {
		response_status, responseBody := financials.GetAriaAccountDetailsAllForClientId(place_holder_map_iu_2["cloud_account_id"], ariaclientId_iu_2, ariaAuth)
		fmt.Println("ariaclientId_iu_2", ariaclientId_iu_2)
		fmt.Println("ariaAuth", ariaAuth)
		fmt.Println("responseBody", responseBody)
		fmt.Println(place_holder_map_iu_2["cloud_account_id"])
	...
	It("Validate Usage calculation for the free product", func() {
		...
		//Expect(nil).To(Equal(err1), "Failed: Validating Usage failed for intel user for small vm product")
	...
	It("Validate Credit depletion", func() {
		...
		response_status, responseBody := financials.GetCredits(baseUrl, token, place_holder_map_iu_2["cloud_account_id"])
		Expect(response_status).To(Equal(200), "Failed to retrieve Billing Account Details from Aria")
		usedAmount := gjson.Get(responseBody, "totalUsedAmount").Float()
		Expect(usedAmount).To(Equal(float64(0)), "Billing Reported Non Zero Credit for Free Product Usage")
	})
	...
	It("Delete the created instance", func() {
		...
		place_holder_map_iu_2["instance_deletion_time"] = strconv.FormatInt(time.Now().UnixMilli(), 10)
	})

	It("Delete cloud account", func() {
		fmt.Println("Delete cloud account")
		url := base_url + "/v1/cloudaccounts"
		delete_Cacc, _ := financials.DeleteCloudAccountById(url, token, place_holder_map_iu_2["cloud_account_id"])
	...
	It("Validating deletion of cloud account by retrieving cloud account details", func() {
		...
		url := base_url + "/v1/cloudaccounts"
		resp_body, _ := financials.GetCloudAccountById(url, token, place_holder_map_iu_2["cloud_account_id"])
