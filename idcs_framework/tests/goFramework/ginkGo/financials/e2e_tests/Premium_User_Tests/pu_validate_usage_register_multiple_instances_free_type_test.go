//go:build pu_vm || All || pu_su_pu
// +build pu_vm All pu_su_pu
...
var _ = Describe("VM Instance", Ordered, Label("Compute-VMaaS-E2E"), func() {
	...
	It("Create vm instance", func() {
		...
		VMName_pu := gjson.Get(create_response_body, "metadata.name").String()
		...
		place_holder_map["paid_instance_name"] = vm_name_pu
	})
	...
	It("Validate instance data after deleting all instances", func() {
		...
		Expect(newpaidProdrunSeconds).Should(BeNumerically(">", 0), "Failed: Validating Free Prod Run seconds Failed")
		...
		// Match Product wise
		Expect(freeProdrunSeconds).To(Equal(newfreeProdrunSeconds), "Failed: Run seconds did not match after deleting free instance")
		Expect(paidProdrunSeconds).Should(BeNumerically("<", newpaidProdrunSeconds), "Failed: Run seconds did not match for paid instance")

		// Match amounts

		Expect(amount).To(Equal(newamount), "Failed: Used Credits did not match after deleting free instance")
		Expect(paidProdamount).Should(BeNumerically("<", newpaidProdamount), "Failed: Used Credits did not match after deleting paid instance")
	})
	...
	It("Delete free instance and validate usage", func() {
		...
		Expect(freerunSeconds).Should(BeNumerically("<", newrunSeconds), "Failed: Validating Usage failed for premium user for paid product, runSeconds did not match")
		Expect(amount).To(Equal(newamount), "Failed: Validating Usage failed for premium user for paid product, runSeconds did not match")
		...
		Expect(err5).To(BeNil(), "Failed: Validating Usage failed for premium user for free product")
		...
		Expect(newrunSeconds).Should(BeNumerically("<", newfreerunSeconds), "Failed: Validating Usage failed for premium user for paid product, runSeconds did not match")
		Expect(newamount).Should(BeNumerically("==", newfreeamount), "Failed: Validating Usage failed for premium user for paid product, runSeconds did not match")
		fmt.Println("Intel user usage data", usage2)
		Expect(usage4).To(Equal(usage2), "Failed: Validating Usage failed for premium user for free product, usages did not match")
