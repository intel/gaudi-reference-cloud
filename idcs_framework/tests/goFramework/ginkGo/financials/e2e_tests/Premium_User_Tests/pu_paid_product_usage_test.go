//go:build pu_vm || All || pu_su_pu || test23
// +build pu_vm All pu_su_pu test23
...
var _ = Describe("VM Instance", Ordered, Label("Compute-VMaaS-E2E"), func() {
	...
	// It("Create paid vm instance when pu user have zero cloud credits", func() {
	...
	// 	Expect(create_response_status).To(Equal(200), "Expected response code on paid instance for pu user should be 200 when cloud credits are non zero")
	// })
