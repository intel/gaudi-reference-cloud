//go:build All || VM || pu_su_iu || CC
// +build All VM pu_su_iu CC
...
var _ = Describe("VM Instance", Ordered, Label("Compute-VMaaS-E2E"), func() {
	...
	It("Create cloud account and add credit card as the default payment option", func() {
		token = "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
		userToken, _ = auth.Get_Azure_Bearer_Token(userName)
		userToken = "Bearer " + userToken
		// cloud account creation
		creditCardPayload := financials_utils.GetCreditCardPayload()
		consoleUrl := compute_utils.GetConsoleUrl()
		_, _, _, _, _, password, _, _, _ := auth.Get_Azure_auth_data_from_config(userName)
		financials.EnrollPremiumUserWithCreditCard(creditCardPayload, userName, password, consoleUrl)
		...
		place_holder_map["cloud_account_id"] = cloudaccId
		cloud_account_created = cloudaccId
