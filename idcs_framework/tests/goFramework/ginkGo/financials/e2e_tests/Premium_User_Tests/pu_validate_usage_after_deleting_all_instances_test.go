//go:build pu_vm || All || pu_su_pu || hi
// +build pu_vm All pu_su_pu hi
...
var _ = Describe("VM Instance", Ordered, Label("Compute-VMaaS-E2E"), func() {
	...
	It("Create free vm instance", func() {
	...
	It("Create paid vm instance", func() {
		...
		instance_endpoint := compute_url + "/v1/cloudaccounts/" + cloud_account_created + "/instances"
		vm_name_pu := "autovm-" + utils.GenerateSSHKeyName(4)
		instance_payload := compute_utils.EnrichInstancePayload(compute_utils.GetInstancePayload(), vm_name_pu, "vm-spr-med", "ubuntu-2204-jammy-v20230122", ssh_publickey_name_created, vnet_created)
		fmt.Println("instance_payload", instance_payload)
	...
	It("Validate instance data after deleting all instances", func() {
		...
		Expect(amount).To(Equal(float64(zeroamt)), "Amount used in credits should be equal to zero for free product use")
		...
		Expect(freeProdrunSeconds).Should(BeNumerically(">", 0), "Failed: Validating Free Prod Run seconds Failed")
		Expect(paidProdamount).Should(BeNumerically(">", float64(zeroamt)), "Failed: Amount used in credits should be greater than zero for paid product use")
		...
		Expect(newamount).To(Equal(float64(zeroamt)), "Amount used in credits should be equal to zero for free product use")
