...
var _ = Describe("Check Enterprise Object Storage", Ordered, Label("Object Storage"), func() {
	...
	It("Create Cloud credits for intel user by redeeming coupons", func() {
		...
		coupon_payload := financials_utils.EnrichCreateCouponPayload(financials_utils.GetCreateCouponPayload(), 100, "eu_user", 1)
	...
	It("Create STaaS Object storage", func() {
		fmt.Println("Starting the Object storage via STaaS API...")
		payload := `{
		  "metadata": {
		...
		create_response_code, create_response_body := financials.CreateObjectStorage(compute_url, token, payload, cloud_account_created)
