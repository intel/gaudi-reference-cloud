...
var _ = Describe("Check Standard User Product Usage BM Small", Ordered, Label("Usages-e2e"), func() {
	instanceType := "bm-emr"
	...
	It("Get the created instance details from poduct catalog and validate", func() {
		...
		Expect(strings.Contains(response_body, `"name":"bm-emr"`)).To(BeTrue(), "Validation failed on instance retrieval")
