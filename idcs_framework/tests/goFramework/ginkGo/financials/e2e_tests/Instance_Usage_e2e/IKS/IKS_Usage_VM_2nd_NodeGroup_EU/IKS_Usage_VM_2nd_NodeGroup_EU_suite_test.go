package IKS_Usage_VM_2nd_NodeGroup_EU_suite_test
...
var _ = BeforeSuite(func() {
	...
	Expect(strings.Contains(cloudaccount_creation_body, `"cloudAccountType":"ACCOUNT_TYPE_ENTERPRISE"`)).To(BeTrue(), "Account type is not Enterprise, Validation Failed.")
	Expect(strings.Contains(cloudaccount_creation_body, `"paidServicesAllowed":true`)).To(BeTrue(), "Allowed paid services is not true, Validation Failed.")
...
func TestIKSUsageVM2ndNodeGroupEUsuitetest(t *testing.T) {
	t.Parallel()
	RegisterFailHandler(Fail)
	RunSpecs(t, "IKS usage VM 2nd nodegroup EU suite Test Suite")
}
