...
var _ = Describe("Check Enterprise User Product Usage VM Small", Ordered, Label("Usages-e2e"), func() {
	...
	It("Get the created instance and validate", func() {
		...
		Expect(place_holder_map["file_size"]).To(Equal("5GB"), "Validation failed on file size retrieval")
	})
