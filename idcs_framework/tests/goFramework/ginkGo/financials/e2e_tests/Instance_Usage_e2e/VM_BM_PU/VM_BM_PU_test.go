...
var _ = Describe("Check Premium User Product Usage", Ordered, Label("Usages-e2e"), func() {
	...
	It("Create paid instance without credits", func() {
	...
	It("Create Cloud credits for premium user by redeeming coupons", func() {
		...
		coupon_payload := financials_utils.EnrichCreateCouponPayload(financials_utils.GetCreateCouponPayload(), 150, "idc_billing@intel.com", 1)
	...
	// CREATE NEW INSTANCE
	It("Create paid test instance with credits", func() {
		fmt.Println("Starting the Instance Creation via API...")
		// form the endpoint and payload
		instance_endpoint := compute_url + "/v1/cloudaccounts/" + cloud_account_created + "/instances"
		bm_name_iu := "autovm-" + utils.GenerateSSHKeyName(4)
		instance_payload := compute_utils.EnrichInstancePayload(compute_utils.GetInstancePayload(), bm_name_iu, instanceType, "ubuntu-2204-jammy-v20230122", ssh_publickey_name_created, vnet_created)
		fmt.Println("instance_payload", instance_payload)
		place_holder_map["instance_type"] = instanceType
		create_response_status, create_response_body = frisby.CreateInstance(instance_endpoint, userToken, instance_payload)
		BMName_iu := gjson.Get(create_response_body, "metadata.name").String()
		Expect(create_response_status).To(Equal(200), "Failed to create instance")
		Expect(bm_name_iu).To(Equal(BMName_iu), "Failed to create instance, response validation failed")
	...
	It("Get the created instance and validate", func() {
		...
		Expect(response_status).To(Equal(200), "Failed to retrieve instance")
