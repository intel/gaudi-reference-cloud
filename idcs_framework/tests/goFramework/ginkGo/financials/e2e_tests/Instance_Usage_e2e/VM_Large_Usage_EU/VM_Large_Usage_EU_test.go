package VM_Large_Usage_EU_test
...
var _ = Describe("Check Enterprise User Product Usage Large Medium", Ordered, Label("Usages-e2e"), func() {
	...
	It("Create paid VM test instance", func() {
	...
	It("Create paid test instance", func() {
		...
		Expect(bm_name_iu).To(Equal(BMName_iu), "Failed to create VM instance, response validation failed")
	...
	It("Validate billing account is created for Enterprise User", func() {
	...
	It("Validate Usage calculation for the paid product and validate credits depletion", func() {
		...
		Eventually(func() bool {
			...
			return false
		}, 2*time.Hour, 40*time.Minute).Should(BeTrue())
