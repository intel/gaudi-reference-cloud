...
var _ = Describe("VM Instance", Ordered, Label("Compute-VMaaS-E2E"), func() {
	...
	It("Create vnet with name", func() {
		...
		Expect(vnet_creation_status).To(Equal(200), "Failed to create VNet, body: "+vnet_creation_body)
	...
	It("Create vm instance", func() {
		for _, region := range gtsRegions {
			fmt.Println("Create vm instance in region: ", region)
			...
			fmt.Println("Response Status", create_response_status)
			fmt.Println("Response body", create_response_body)
			VMName_iu := gjson.Get(create_response_body, "metadata.name").String()
			Expect(create_response_status).NotTo(Equal(200), "Failed to create VM instance with wrong GTS, body: "+create_response_body)
			Expect(vm_name).NotTo(Equal(VMName_iu), "Failed to create VM instance, response validation failed")
