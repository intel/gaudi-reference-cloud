...
var _ = Describe("STaaS Instance", Ordered, Label("Valid GTS StaaS"), func() {
	var base_url string
	...
	It("Create STaaS Instance", func() {
		...
		fmt.Println("staas_url: ", base_url)
		staas_payload = compute_utils.GetStaaSPayload("500GB", "")
		fmt.Println("staas_payload: ", staas_payload)

		create_response_code, create_response_body := financials.CreateFileSystem(compute_url, userToken, staas_payload, cloud_account_created)
		instance_id_created = gjson.Get(create_response_body, "metadata.resourceId").String()
		fmt.Println("resourceId: ", instance_id_created)
		Expect(create_response_code).To(Or(Equal(200), Equal(403)), "Failed to create FileSystem, body: "+create_response_body)
	})
})
