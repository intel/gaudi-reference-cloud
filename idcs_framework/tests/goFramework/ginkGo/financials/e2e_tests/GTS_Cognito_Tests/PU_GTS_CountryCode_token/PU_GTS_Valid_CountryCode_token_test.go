...
var _ = Describe("VM Instance", Ordered, Label("Valid GTS VMaaS"), func() {
	var base_url string
	...
	It("Create vm instance", func() {
		...
		Expect(create_response_status).To(Equal(200), "Failed to create VM instance, body: "+create_response_body)
		Expect(vm_name).To(Equal(VMName_iu), "Failed to create VM instance, response validation failed")
