	...
	It("Create IKS cluster", func() {
		if !strings.Contains(compute_url, "/staging") {
			...
			for _, region := range gtsRegions {
				...
				response_status, responseBody := iks.CreateSCCluster(baseUrl, token, payload)
				logger.Logf.Info("Response body", responseBody)
				Expect(response_status).NotTo(Equal(200), "Failed, SC instance was created with wrong GTS")
