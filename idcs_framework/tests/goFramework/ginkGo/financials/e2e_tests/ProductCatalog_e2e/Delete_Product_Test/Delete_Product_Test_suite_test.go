...
var _ = BeforeSuite(func() {
	...
	By("Starting product delete process...")
	error := ExecDeleteProductCommand("product")
	fmt.Print(error)
	//Expect(error.Error()).To(BeNil(), "Failed to execute command, error: "+error.Error())
	logger.Logf.Info("Product Deleted in current context.")
})

func TestDeleteProductTest(t *testing.T) {
	t.Parallel()
	RegisterFailHandler(Fail)
	RunSpecs(t, "DeleteProductTest Suite")
}
