...
var _ = BeforeSuite(func() {
	...
	fmt.Println("OIDC TOKEN....", userToken)
	...
	Expect(strings.Contains(string(out), `"type":"ACCOUNT_TYPE_ENTERPRISE"`)).To(BeTrue(), "Validation failed on Enrollment of Intel user")
	place_holder_map["cloud_account_id"] = cloud_account_created
	place_holder_map["cloud_account_type"] = cloudaccount_type
	// Enroll new Enterprise Account to be compliant

	fmt.Println("Enrolled....", create_payload.Enrolled)
	ret_value, respBody := cloudAccounts.GetCAccEUEnrolled(base_url, token, cloudaccount_type, create_payload.Enrolled, 200)

	fmt.Println("Enroll value", ret_value)
...
func TestEnterpriseUserTests(t *testing.T) {
	t.Parallel()
	RegisterFailHandler(Fail)
	RunSpecs(t, "EnterpriseUserTests Suite")
}
