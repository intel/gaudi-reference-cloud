...
var _ = Describe("VM Instance", Ordered, Label("Compute-VMaaS-E2E"), func() {
	...
	It("Validate billing account is Created for Standard User", func() {
		response_status, responseBody := financials.GetAriaAccountDetailsAllForClientId(place_holder_map["cloud_account_id"], ariaclientId_su, ariaAuth)
	...
	It("Delete free instance and validate usage is not increasing", func() {
		...
		fmt.Println("Waiting for sometime after deleting instance....")
		...
		Expect(runSeconds).To(Equal(newrunSeconds), "Failed: Validating Usage failed for Standard user for free product, runSeconds did not match")
		Expect(amount).To(Equal(newamount), "Failed: Validating Usage failed for Standard user for free product, runSeconds did not match")
