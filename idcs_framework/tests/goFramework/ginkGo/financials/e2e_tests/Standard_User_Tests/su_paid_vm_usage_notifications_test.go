//go:build su_vm_80 || All || pu_su_iu
// +build su_vm_80 All pu_su_iu
...
var _ = Describe("VM Instance", Ordered, Label("Compute-VMaaS-E2E"), func() {
	var create_response_status_iu_paid_1 int
	var create_response_body_iu_paid_1 string
	var place_holder_map2_paid_1 = make(map[string]string)
	var meta_data_map2_paid_1 = make(map[string]string)
	var ariaclientId_iu_paid_1 string
	...
	BeforeAll(func() {
		...
		ariaclientId_iu_paid_1 = financials_utils.GetAriaClientNo()
		...
		fmt.Println("ariaclientId_iu_paid_1", ariaclientId_iu_paid_1)
	})
	...
	It("Delete cloud account", func() {
		...
		if err == nil {
			code, body := financials.DeleteCloudAccountById(url, token, cloudAccId)
			fmt.Println("Cloud account deleted with status code", code)
			fmt.Println("Cloud account deleted with response body", body)
		}
	...
	It("Create cloud account", func() {
		...
		place_holder_map2_paid_1["cloud_account_id"] = cloud_account_created
		place_holder_map2_paid_1["cloud_account_type"] = cloudaccount_type
	...
	// 	place_holder_map2_paid_1["instance_type"] = "vm-spr-med"
	// 	// hit the api
	// 	create_response_status_iu_paid_1, create_response_body_iu_paid_1 = frisby.CreateInstance(instance_endpoint, userToken, instance_payload)
	// 	Expect(create_response_status_iu_paid_1).To(Equal(403), "Expected response code on paid instance for Standard user should be 403 when cloud credits are zero")
	// 	Expect(strings.Contains(create_response_body_iu_paid_1, `"message":"paid service not allowed"`)).To(BeTrue(), "Failed to validate ")
	...
	It("Create Cloud credits for Standard user by redeeming coupons", func() {
		...
		coupon_payload := financials_utils.EnrichCreateCouponPayload(financials_utils.GetCreateCouponPayloadStandard(), 0.1, "idc_billing@intel.com", 1)
		...
		redeem_payload := financials_utils.EnrichRedeemCouponPayload(financials_utils.GetRedeemCouponPayload(), gjson.Get(coupon_creation_body, "code").String(), place_holder_map2_paid_1["cloud_account_id"])
	...
	It("Create paid vm instance", func() {
		...
		// hit the api
		create_response_status_iu_paid_1, create_response_body_iu_paid_1 = frisby.CreateInstance(instance_endpoint, userToken, instance_payload)
		VMName_iu := gjson.Get(create_response_body_iu_paid_1, "metadata.name").String()
		Expect(create_response_status_iu_paid_1).To(Equal(200), "Failed to create VM instance")
		Expect(vm_name_iu).To(Equal(VMName_iu), "Failed to create VM instance, resposne validation failed")
		place_holder_map2_paid_1["instance_creation_time"] = strconv.FormatInt(time.Now().UnixMilli(), 10)
	})
	...
	It("Get the created instance and validate", func() {
		...
		instance_id_created = gjson.Get(create_response_body_iu_paid_1, "metadata.resourceId").String()
		...
		place_holder_map2_paid_1["resource_id"] = instance_id_created
		place_holder_map2_paid_1["machine_ip"] = gjson.Get(response_body, "status.interfaces[0].addresses[0]").String()
		fmt.Println("IP Address is :" + place_holder_map2_paid_1["machine_ip"])
	})
	...
	// 	inventory_generated := compute_utils.EnrichInventoryData(inventory_raw_data, proxyIp, place_holder_map2_paid_1["machine_ip"], "~/.ssh/id_rsa")
	...
	It("Get the created instance details from poduct catalog and validate", func() {
		...
		meta_data_map2_paid_1["Highlight"] = structResponse.Products[0].Metadata.Highlight
		meta_data_map2_paid_1["disks.size"] = structResponse.Products[0].Metadata.Disks
		meta_data_map2_paid_1["instanceType"] = structResponse.Products[0].Metadata.InstanceType
		meta_data_map2_paid_1["memory.size"] = structResponse.Products[0].Metadata.Memory
		meta_data_map2_paid_1["region"] = structResponse.Products[0].Metadata.Region
		fmt.Println("meta_data_map2_paid_1", meta_data_map2_paid_1)
	})
	...
	It("Get Metering data related to product and validate", func() {
		search_payload := financials_utils.EnrichMmeteringSearchPayload(compute_utils.GetMeteringSearchPayload(),
			place_holder_map2_paid_1["resource_id"], place_holder_map2_paid_1["cloud_account_id"])
		...
		Expect(response_status).To(Equal(200), "Failed to retrieve Product Details")
		Expect(place_holder_map2_paid_1["cloud_account_id"]).To(Equal(metering_record_cloudAccountId), "Validation failed on cloud account id retrieval")
		Expect(place_holder_map2_paid_1["resource_id"]).To(Equal(metering_record_resourceId), "Validation failed on resource id retrieval")
		Expect(meta_data_map2_paid_1["instanceType"]).To(Equal(metering_record_instanceType), "Validation failed on instance retrieval")
		place_holder_map2_paid_1["transactionId"] = metering_record_transactionId
		place_holder_map2_paid_1["reported"] = metering_record_reported
	})

	It("Validate billing account is not Created for Standard User", func() {
		response_status, responseBody := financials.GetAriaAccountDetailsAllForClientId(place_holder_map2_paid_1["cloud_account_id"], ariaclientId_iu_paid_1, ariaAuth)
		fmt.Println("ariaclientId_iu_paid_1", ariaclientId_iu_paid_1)
		fmt.Println("ariaAuth", ariaAuth)
		fmt.Println("responseBody", responseBody)
		fmt.Println(place_holder_map2_paid_1["cloud_account_id"])
	...
	// 	place_holder_map2_paid_1["instance_deletion_time"] = strconv.FormatInt(time.Now().UnixMilli(), 10)
	// })
	...
	It("Validate Credit Depletion", func() {
		...
		response_status, responseBody := financials.GetCredits(baseUrl, token, place_holder_map2_paid_1["cloud_account_id"])
		//Expect(response_status).To(Equal(200), "Failed to retrieve Billing Account Details from Aria")
	...
	It("Validate Notifications sent to user for 80% and 100% usage", func() {
		response_status, responseBody := financials.GetNotificationsShortPoll(base_url, token, place_holder_map2_paid_1["cloud_account_id"])
		...
		notificationArray.ForEach(func(key, value gjson.Result) bool {
			...
			Expect(place_holder_map2_paid_1["cloud_account_id"]).To(Equal(cloudAccId), "Validation failed on cloud account id in notifications")
	...
	It("Delete cloud account", func() {
		fmt.Println("Delete cloud account")
		url := base_url + "/v1/cloudaccounts"
		delete_Cacc, _ := financials.DeleteCloudAccountById(url, token, place_holder_map2_paid_1["cloud_account_id"])
	...
	It("Validating deletion of cloud account by retrieving cloud account details", func() {
		...
		url := base_url + "/v1/cloudaccounts"
		resp_body, _ := financials.GetCloudAccountById(url, token, place_holder_map2_paid_1["cloud_account_id"])
