//go:build su_vm || All || pu_su
// +build su_vm All pu_su
...
var _ = Describe("VM Instance", Ordered, Label("Compute-VMaaS-E2E"), func() {
	...
	It("Create vm instance", func() {
		...
		Expect(vm_name).To(Equal(VMName), "Failed to create VM instance, resposne validation failed")
	...
	It("Create paid vm instance", func() {
		...
		create_response_status, create_response_body = frisby.CreateInstance(instance_endpoint, userToken, instance_payload)
		VMName := gjson.Get(create_response_body, "metadata.name").String()
		place_holder_map["paid_instance_name"] = vm_name
		Expect(create_response_status).To(Equal(200), "Failed to create VM instance")
		Expect(vm_name).To(Equal(VMName), "Failed to create VM instance, resposne validation failed")
		place_holder_map["instance_creation_time"] = strconv.FormatInt(time.Now().UnixMilli(), 10)
	})
	...
	It("Validate Usage calculation for the paid product and validate credits depletion", func() {
		...
		fmt.Println("Usage of free instance ", usage)
		...
		// Expect(usage).To(Equal(usedAmount), "Billing Reported Non Zero Credit for Free Product Usage")

	})

	It("Delete paid instance and validate usage is not increasing", func() {
		token = "Bearer " + auth.Get_Azure_Admin_Bearer_Token()
		userToken, _ = auth.Get_Azure_Bearer_Token(userName)
		userToken = "Bearer " + userToken
		instance_endpoint := compute_url + "/v1/cloudaccounts/" + cloud_account_created + "/instances"
		...
		Expect(err1).To(BeNil(), "Failed: Validating Usage failed for Standard user for free product")
		runSeconds := testsetup.GetProductRunSec(userName, "vm-spr-med")
		amount := testsetup.GetProductUsage(userName, "vm-spr-med")
		freerunSeconds := testsetup.GetProductRunSec(userName, "vm-spr-sml")
		freeamount := testsetup.GetProductUsage(userName, "vm-spr-sml")
		time.Sleep(5 * time.Minute)
		usage4, err4 := testsetup.GetUsageAndValidateTotalUsage(userName, resourceInfo, base_url, token, compute_url)
		Expect(err4).To(BeNil(), "Failed: Validating Usage failed for Standard user for free product")
		newrunSeconds := testsetup.GetProductRunSec(userName, "vm-spr-med")
		newamount := testsetup.GetProductUsage(userName, "vm-spr-med")
		Expect(runSeconds).To(Equal(newrunSeconds), "Failed: Validating Usage failed for Standard user for paid product, runSeconds did not match")
		Expect(amount).To(Equal(newamount), "Failed: Validating Usage failed for Standard user for paid product, runSeconds did not match")
		// delete_response_status, _ = frisby.DeleteInstanceById(instance_endpoint, token, instance_id_created1)
		// Expect(delete_response_status).To(Equal(200), "Failed to delete VM instance")
		time.Sleep(1 * time.Minute)
		_, err5 := testsetup.GetUsageAndValidateTotalUsage(userName, resourceInfo, base_url, token, compute_url)
		Expect(err5).To(BeNil(), "Failed: Validating Usage failed for Standard user for free product")
		...
		Expect(freerunSeconds).Should(BeNumerically("<", newfreerunSeconds), "Failed: Validating Usage failed for Standard user for paid product, runSeconds did not match")
		Expect(freeamount).To(Equal(newfreeamount), "Failed: Validating Usage failed for Standard user for paid product, runSeconds did not match")
		fmt.Println("Standard user usage data", usage2)
		Expect(usage4).To(Equal(usage2), "Failed: Validating Usage failed for Standard user for free product, usages did not match")
	})
