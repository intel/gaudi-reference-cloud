//go:build su_vm || All || VM1 || pu_su_iu
// +build su_vm All VM1 pu_su_iu
...
var su_create_response_status int
var su_create_response_body string
var su_place_holder_map = make(map[string]string)
var su_meta_data_map = make(map[string]string)
var su_ariaclientId_su string
...
var _ = Describe("VM Instance", Ordered, Label("Compute-VMaaS-E2E"), func() {
	...
	BeforeAll(func() {
		...
		su_ariaclientId_su = financials_utils.GetAriaClientNo()
		...
		fmt.Println("su_ariaclientId_su", su_ariaclientId_su)
	})
	...
	It("Create cloud account", func() {
		...
		su_place_holder_map["cloud_account_id"] = cloud_account_created
		su_place_holder_map["cloud_account_type"] = cloudaccount_type
	...
	It("Create vm instance", func() {
		...
		su_place_holder_map["instance_type"] = instanceType

		// hit the api
		su_create_response_status, su_create_response_body = frisby.CreateInstance(instance_endpoint, token, instance_payload)
		VMName_iu := gjson.Get(su_create_response_body, "metadata.name").String()
		Expect(su_create_response_status).To(Equal(200), "Failed to create VM instance")
		Expect(vm_name).To(Equal(VMName_iu), "Failed to create VM instance, resposne validation failed")
		//Expect(strings.Contains(su_create_response_body, `"name":"e2e-suvm1"`)).To(BeTrue(), "Failed to create VM instance, resposne validation failed")
		su_place_holder_map["instance_creation_time"] = strconv.FormatInt(time.Now().UnixMilli(), 10)
	})
	...
	It("Get the created instance and validate", func() {
		...
		instance_id_created = gjson.Get(su_create_response_body, "metadata.resourceId").String()
		...
		su_place_holder_map["resource_id"] = instance_id_created
		su_place_holder_map["machine_ip"] = gjson.Get(response_body, "status.interfaces[0].addresses[0]").String()
		fmt.Println("IP Address is :" + su_place_holder_map["machine_ip"])
	})
	...
	// 	inventory_generated := compute_utils.EnrichInventoryData(inventory_raw_data, proxyIp, "guest", su_place_holder_map["machine_ip"], "~/.ssh/id_rsa")
	...
	It("Delete the created instance", func() {
		...
		su_place_holder_map["instance_deletion_time"] = strconv.FormatInt(time.Now().UnixMilli(), 10)
	})
	...
	It("Get the created instance details from product catalog and validate", func() {
		...
		su_meta_data_map["Highlight"] = structResponse.Products[0].Metadata.Highlight
		su_meta_data_map["disks.size"] = structResponse.Products[0].Metadata.Disks
		su_meta_data_map["instanceType"] = structResponse.Products[0].Metadata.InstanceType
		su_meta_data_map["memory.size"] = structResponse.Products[0].Metadata.Memory
		su_meta_data_map["region"] = structResponse.Products[0].Metadata.Region
		fmt.Println("su_meta_data_map", su_meta_data_map)
	})
	...
	It("Get Metering data related to product and validate", func() {
		search_payload := financials_utils.EnrichMmeteringSearchPayload(compute_utils.GetMeteringSearchPayload(),
			su_place_holder_map["resource_id"], su_place_holder_map["cloud_account_id"])
		...
		Expect(response_status).To(Equal(200), "Failed to retrieve Product Details")
		Expect(su_place_holder_map["cloud_account_id"]).To(Equal(metering_record_cloudAccountId), "Validation failed on cloud account id retrieval")
		Expect(su_place_holder_map["resource_id"]).To(Equal(metering_record_resourceId), "Validation failed on resource id retrieval")
		Expect(su_meta_data_map["instanceType"]).To(Equal(metering_record_instanceType), "Validation failed on instance retrieval")
		su_place_holder_map["transactionId"] = metering_record_transactionId
		su_place_holder_map["reported"] = metering_record_reported
	})

	// billing
	// Make sure billing account is not created for Standard user
	It("Validate billing account is Created for Standard User", func() {
		response_status, responseBody := financials.GetAriaAccountDetailsAllForClientId(su_place_holder_map["cloud_account_id"], su_ariaclientId_su, ariaAuth)
	...
	It("Delete cloud account", func() {
		fmt.Println("Delete cloud account")
		url := base_url + "/v1/cloudaccounts"
		delete_Cacc, _ := financials.DeleteCloudAccountById(url, token, su_place_holder_map["cloud_account_id"])
	...
	It("Validating deletion of cloud account by retrieving cloud account details", func() {
		...
		url := base_url + "/v1/cloudaccounts"
		resp_body, _ := financials.GetCloudAccountById(url, token, su_place_holder_map["cloud_account_id"])
