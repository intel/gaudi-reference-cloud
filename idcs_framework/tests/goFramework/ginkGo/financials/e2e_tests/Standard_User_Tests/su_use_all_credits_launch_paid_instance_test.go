//go:build su_vm || All || pu_su_iu || iu_free
// +build su_vm All pu_su_iu iu_free
...
var _ = Describe("VM Instance", Ordered, Label("Compute-VMaaS-E2E"), func() {
	...
	It("Create Cloud credits for Standard user by redeeming coupons", func() {
		...
		coupon_payload := financials_utils.EnrichCreateCouponPayload(financials_utils.GetCreateCouponPayloadStandard(), 1, "idc_billing@intel.com", 1)
	...
	It("Create paid vm instance when cloud account has no credits", func() {
		...
		Expect(create_response_status).To(Equal(403), "Paid instance created after Standard user used all the credits")
	})
