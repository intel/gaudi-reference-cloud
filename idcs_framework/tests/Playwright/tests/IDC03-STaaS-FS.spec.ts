...
function IDC03_TEST_PLAN (env: string, reg: string, utype: string) {
   console.log(`
      Scenario IDC03-STaaS-FS:
          ...
          Create Storage/Filesystem (30-sec timeout)
          Generate password, verify all statuses
          Read all mounting/unmounting instructions
          Verify SSH connectivity
          Make remote Weka install (30-sec timeout)
          Make remote Weka login using creds (30-sec timeout)
          Make remote Weka mount (7-min timeout)
          Make remote mount verifications (df, mount, weka status)
          Make remote umount

          Delete File storage in UI
...
test.describe('IDC03-STaas-FS', () => {
  ...
  for (const testedRegion of testedRegions) {
    for (const testedUserType of testedUserTypes) {
      ...
      test(`IDC03-${idcRegionsShorts[testedRegion]}-${idcUsersShorts[testedUserType]}`, async ({ browser }, testInfo) => {
        
        test.setTimeout(660000)  // 11-min timeout for this test
