...
const ProductCatalogEditMetaDatasetContainer = (): JSX.Element => {
  // *****
  // Params
  // *****
  ...
  const copyTemplateModalInitial = {
    show: false,
    title: 'Copy metadatasets from template'
  }
  ...
  const updateMetaDataSetGridFromStore = (): void => {
    if (editProduct) {
      ...
      const metadatasets = [...editProduct.metaDataSets]
      metadatasets.forEach(item => {
        ...
        for (const indexMeta in metaDataSet.metaData) {
          ...
          metaDataItems.push({
            ...
            type: metaData.type,
            context: metaData.metadataSetId
          })
        }
      })
  ...
  const updateMetaDataSetGridFromTemplate = (): void => {
    ...
    for (const index in product?.metaDataSets) {
      ...
      if (metaDataSet.metaData) {
        const metaDataCopy = [...metaDataSet.metaData]
        metaDataCopy.forEach(item => {
          const metaData = { ...item }
          metaDataItems.push({
            id: metaData.id,
            key: metaData.key,
  ...
  const updateMetaDataGridFromTemplate = (): void => {
    ...
    metaDataSelectedItems.forEach(item => {
      const metaData = { ...item }
      if (!String(metaData.id).includes('delete')) {
  ...
  const deleteMetadataGridFromManual = (): void => {
    ...
    productMetaDataItems.forEach((meta) => {
      if (meta.id === metaDataItem.id) {
        ...
        if (String(meta.id).includes('new')) {
          return
        }
        rowUpdated.id = `delete-${meta.id}`
  ...
  const addMetaDataItem = (): void => {
    ...
    productMetaDataItemsUpdated.push({
      id: `new-${productMetaDataItemsUpdated.length + 1}`,
  ...
  const editMetaDataItem = (): void => {
    ...
    productMetaDataItems.forEach((meta) => {
      if (meta.id === metaDataItem.id) {
        const rowUpdated = { ...meta }
        if (String(meta.id).includes('edit-')) {
          rowUpdated.id = meta.id
        } else {
          rowUpdated.id = `edit-${meta.id}`
        }
  ...
  const saveAllChanges = (): void => {
    ...
    updatedProduct.metaDataSets = [...metaDataSets]
...
export default ProductCatalogEditMetaDatasetContainer
