...
sed -i "/version:/ s/$/-${GIT_SHORT_VERSION}/" charts/idcs-admin-portal/Chart.yaml

# package idcs helm chart
helm package charts/idcs-admin-portal || { echo 'helm package failed' ; exit 1; }
...
# push chart to repo
helm push idcs-admin-portal-${HELM_CHART_VERSION}.tgz oci://${REGISTRY}/${PROJECT} || { echo 'helm push to harbor repo failed' ; exit 1; }
