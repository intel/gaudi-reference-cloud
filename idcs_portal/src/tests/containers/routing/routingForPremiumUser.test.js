...
import { mockPaymentServices } from '../../mocks/consoleUIs/ConsoleUIs'
...
describe('Routes Premium User', () => {
  ...
  it('Check Premium users have access to the home page', async () => {
    history.push('/')
    render(<TestComponent history={history} />)
    expect(await screen.findByTestId('HomePage')).toBeVisible()
  })

  it('Check Premium users have access to hardware page', async () => {
    history.push('/hardware')
    render(<TestComponent history={history} />)
    expect(await screen.findByTestId('/hardware')).toBeVisible()
  })

  it('Check Premium users have access to training page', async () => {
    history.push('/learning/notebooks')
    render(<TestComponent history={history} />)
    expect(await screen.findByTestId('/learning/notebooks')).toBeVisible()
  })

  it('Check Premium users have access to public keys page', async () => {
    history.push('/security/publickeys')
    render(<TestComponent history={history} />)
    expect(await screen.findByTestId('/security/publickeys')).toBeVisible()
  })

  it('Check Premium users have access to public keys import page', async () => {
    history.push('/security/publickeys/import')
    render(<TestComponent history={history} />)
    expect(await screen.findByTestId('/security/publickeys/import')).toBeVisible()
  })

  it('Check Premium users have access to error page', async () => {
    history.push('/error/notfound')
    render(<TestComponent history={history} />)
    expect(await screen.findByText('Page not found')).toBeVisible()
  })

  it('Check Premium users have access to access denied page', async () => {
    history.push('/error/accessdenied')
    render(<TestComponent history={history} />)
    expect(await screen.findByText('Your account is being verified.')).toBeVisible()
  })

  it('Check Premium users have access to something went wrong page', async () => {
    history.push('/error/somethingwentwrong')
    render(<TestComponent history={history} />)
    expect(await screen.findByText('Something went wrong')).toBeVisible()
  })

  it('Check Premium users have access to compute reserve page', async () => {
    history.push('/compute/reserve')
    render(<TestComponent history={history} />)
    expect(await screen.findByTestId('/compute/reserve')).toBeVisible()
  })

  it('Check Premium users have access to compute edit page', async () => {
    history.push('/compute/d/test/edit')
    render(<TestComponent history={history} />)
    expect(await screen.findByTestId('/compute/d/test/edit')).toBeVisible()
  })

  it('Check Premium users have access to compute page', async () => {
    history.push('/compute')
    render(<TestComponent history={history} />)
    expect(await screen.findByTestId('/compute')).toBeVisible()
  })

  it('Check Premium users have access to compute-groups reserve page', async () => {
    history.push('/compute-groups/reserve')
    render(<TestComponent history={history} />)
    expect(await screen.findByTestId('/compute-groups/reserve')).toBeVisible()
  })

  it('Check Premium users have access to compute-groups edit page', async () => {
    history.push('/compute-groups/d/test/edit')
    render(<TestComponent history={history} />)
    expect(await screen.findByTestId('/compute-groups/d/test/edit')).toBeVisible()
  })

  it('Check Premium users have access to compute-groups my reservations page', async () => {
    history.push('/compute-groups')
    render(<TestComponent history={history} />)
    expect(await screen.findByTestId('/compute-groups')).toBeVisible()
  })

  it('Check Premium users have access to storage reserve page', async () => {
    history.push('/storage/reserve')
    render(<TestComponent history={history} />)
    expect(await screen.findByText('/storage/reserve')).toBeVisible()
  })

  it('Check Premium users have access to storage page', async () => {
    history.push('/storage')
    render(<TestComponent history={history} />)
    expect(await screen.findByText('/storage')).toBeVisible()
  })

  it('Check Premium users have access to invoices page', async () => {
    history.push('/billing/invoices')
    render(<TestComponent history={history} />)
    expect(await screen.findByTestId('/billing/invoices')).toBeVisible()
  })

  it('Check Premium users have access to credits page', async () => {
    history.push('/billing/credits')
    render(<TestComponent history={history} />)
    expect(await screen.findByTestId('/billing/credits')).toBeVisible()
  })

  it('Check Premium users have access to manage payment methods page', async () => {
    history.push('/billing/managePaymentMethods')
    render(<TestComponent history={history} />)
    expect(await screen.findByTestId('/billing/managePaymentMethods')).toBeVisible()
  })

  it('Check Premium users have access to manage credit card page', async () => {
    history.push('/billing/managePaymentMethods/managecreditcard')
    render(<TestComponent history={history} />)
    expect(await screen.findByTestId('/billing/managePaymentMethods/managecreditcard')).toBeVisible()
  })

  it('Check Premium users have access to manage coupon code page', async () => {
    history.push('/billing/credits/managecouponcode')
    render(<TestComponent history={history} />)
    expect(await screen.findByTestId('/billing/credits/managecouponcode')).toBeVisible()
  })

  it('Check Premium users have access to credit response page', async () => {
    history.push('/billing/creditResponse')
    render(<TestComponent history={history} />)
    expect(await screen.findByTestId('/billing/creditResponse')).toBeVisible()
  })

  it('Check Premium users have access to premium page', async () => {
    history.push('/premium')
    render(<TestComponent history={history} />)
    expect(await screen.findByTestId('/premium')).toBeVisible()
  })

  it('Check Premium users have access to upgrade account', async () => {
  ...
  it('Check Premium users have access to KaaS Create Cluster page', async () => {
    history.push('/cluster/reserve')
    render(<TestComponent history={history} />)
    expect(await screen.findByText('/cluster/reserve')).toBeVisible()
  })

  it('Check Premium users have access to KaaS My Clusters page in root', async () => {
    history.push('/cluster')
    render(<TestComponent history={history} />)
    expect(await screen.findByText('/cluster')).toBeVisible()
  })

  it('Check Premium users have access to KaaS Load Balancers reservation page', async () => {
  ...
  it('Check Premium users have access to KaaS Node Group creation page', async () => {
  ...
  it('Check Premium users have NOT access with future flag disable to profile apikeys page', async () => {
